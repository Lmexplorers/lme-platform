#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Mia & Teo skoledagbok 1.-3. trinn - HELE boka, trykkeklar A5 staaende, skoleaaret 2026/2027."""
import os
import datetime
...
OUT = os.environ.get("DIARY_OUT", os.path.join(_BASE, "Mia-og-Teo-skoledagbok-2026-2027-A5.pdf"))
...
UNDRING=[
 "Hvor mange forskjellige blader klarer du å finne på vei hjem fra skolen?",
 "Hvilke lyder hører du når du står helt stille ute? Tell dem.",
 "Hva er det første du legger merke til når du går ut i dag?",
 "Finn noe som er rundt, og noe som er spisst.",
 "Hvilken farge ser du mest av ute i dag?",
 "Hvor mange bein hadde det siste dyret du så? Tegn det.",
 "Kjenn på tre forskjellige overflater. Hvilken likte du best?",
 "Hva tror du fuglene snakker om?",
 "Finn en stein du synes er fin. Hvorfor valgte du akkurat den?",
 "Hvor kommer regnet fra, tror du?",
 "Hvilket tre er størst der du bor?",
 "Lukt på noe ute. Hva minner lukten deg om?",
 "Hvor mange skritt er det fra døra di til nærmeste tre?",
 "Se opp. Hva ser du på himmelen akkurat nå?",
 "Hva skjer med en sølepytt når sola kommer fram?",
 "Finn noe mykt og noe hardt i naturen.",
 "Hvilket lite dyr vil du gjerne vite mer om?",
 "Hvor sover fuglene om natten, tror du?",
 "Finn noe som er glatt, og noe som er ruglete.",
 "Tell hvor mange trær du går forbi på vei hjem.",
 "Hvilken vei blåser vinden i dag?",
 "Finn to blader som ikke er helt like.",
 "Hva tror du skjer med bladene om høsten?",
 "Hvor mange farger finner du på ett eneste blad?",
 "Hva er det fineste du så i dag?",
 "Hvis du var en maur, hvor ville du gått?",
 "Lær deg navnet på en blomst du ikke kunne fra før.",
 "Finn en skygge. Hva er det som lager den?",
 "Hvor varmt eller kaldt er det ute i dag? Gjett først, sjekk etterpå.",
 "Hvilket dyr skulle du ønske bodde i hagen din?",
 "Hva tror du er under en stor stein? Se forsiktig etter.",
 "Finn noe ute som er eldre enn deg.",
 "Hvor mange fugler ser du på vei hjem?",
 "Hva lager spor i snøen eller i sølen?",
 "Hvilken årstid liker du best, og hvorfor?",
 "Hva ville du spurt et tre om hvis det kunne svare?",
 "Hvor mange forskjellige grønnfarger ser du ute?",
 "Lytt etter vann. Hører du noe?",
 "Hva tror du insektene gjør når det regner?",
 "Finn det minste du klarer å se uten forstørrelsesglass.",
 "Hva er det rareste du har sett i naturen?",
 "Tegn skyen som ligner mest på noe.",
 "Hvilke bær eller frukter vokser der du bor?",
 "Hva skjer med et frø som havner i jorda?",
 "Finn noe naturen har laget, og noe mennesker har laget.",
 "Hva gjør deg aller mest nysgjerrig akkurat nå?",
 "Hvor mange forskjellige fuglelyder klarer du å skille?",
 "Hvis du kunne være ute hele dagen, hva ville du utforsket?",
]
WUNDRING=[
...
def cover():
    return f'''<div class="page cover">
      ...
      <div class="cgrade">1.–3. trinn</div>
      <div class="cage">(5–9 år)</div>
...
def school_day(d,uke,uidx):
    return f'''<div class="page">
      ...
      {moodrow()}
      <div class="card school"><div class="flabel">På skolen i dag</div>{lines(5)}</div>
      <div class="card reflect">
        <div class="rrow"><div class="flabel">Dagens oppdagelse</div><span class="rline"></span></div>
        <div class="rrow"><div class="flabel">Dagens gode gjerning</div><span class="rline"></span></div>
        <div class="rrow"><div class="flabel">Noe jeg er stolt av i dag</div><span class="rline"></span></div>
      </div>
      <div class="undring"><div class="txt"><div class="ut">Dagens undring</div><div class="q">{UNDRING[uidx%len(UNDRING)]}</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def weekend_day(d,uke,widx):
    return f'''<div class="page">
      ...
      <div class="undring"><div class="txt"><div class="ut">Dagens undring</div><div class="q">{WUNDRING[widx%len(WUNDRING)]}</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''

FERIE_UNDRING=["Hva er det beste med å ha fri fra skolen?","Fant du på noe gøy ute i dag?",
 "Hvem var du sammen med i dag?","Hva vil du gjøre i morgen?","Hva fant du på helt utenom skolen i dag?",
 "Hva er du mest takknemlig for i ferien?"]
def ferie_day(d,uke,navn,fidx):
    return f'''<div class="page">
      {band(navn, f"Ferie &nbsp;·&nbsp; {datestr(d)}")}
      {moodrow()}
      <div class="card"><div class="flabel">Det jeg fant på i dag</div>{lines(4)}</div>
      <div class="card draw"><div class="flabel">Tegn noe fra dagen</div></div>
      <div class="undring"><div class="txt"><div class="ut">Ferieundring</div><div class="q">{FERIE_UNDRING[fidx%len(FERIE_UNDRING)]}</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def p_utebingo(s):
    ...
    return f'''<div class="page">
      ...
      <div class="undring tip"><div class="txt"><div class="ut">Lite tips</div><div class="q">Du trenger ikke finne alt på en gang. Naturen er der hver dag og venter på deg.</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def p_forsker(s):
    ...
    return f'''<div class="page">
      ...
      <div class="undring tip"><div class="txt"><div class="ut">Visste du at</div><div class="q">Et lite frø har alt det trenger inni seg for å bli en plante. Det venter bare på jord, vann og lys.</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def p_takk(s):
    return f'''<div class="page">
      ...
      <div class="undring"><div class="txt"><div class="ut">Mia &amp; Teo sier</div><div class="q">Små gode ting teller også. En god klem, sola i ansiktet, eller noe godt å spise.</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def p_telle(s, tip=0):
    ...
    return f'''<div class="page">
      ...
      <div class="undring tip"><div class="txt"><div class="ut">Visste du at</div><div class="q">{TELLE_TIPS[tip%len(TELLE_TIPS)]}</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
...
def p_stille(s):
    return f'''<div class="page">
      ...
      <div class="undring tip"><div class="txt"><div class="ut">Lite tips</div><div class="q">Det er godt for hjernen å hvile litt. Du trenger ikke gjøre noe, bare være.</div></div><img src="file://{A}/mia-og-teo.png"></div>
    </div>'''
