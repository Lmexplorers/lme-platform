...
(function () {
  ...
  make.book = function (cfg, onProgress) {
    ...
    var facts = 'Title: ' + cfg.title + '\nTopic: ' + (cfg.topic || '-') + '\nAudience: ' + (cfg.audience || '-') +
      ...
      '\nORIGINALITY RULES: This brand already has books using the formula "the characters walk through nature and greet one new animal per page" (Naturutforskerne, the Easter book). Do NOT reuse that formula, its structure or its phrasing. Do NOT reuse the animal cast from those books either (butterfly, ladybug, caterpillar, snail, ant, frog, bluebird, Easter chicks and bunnies); pick animals and creatures that genuinely belong to THIS topic and setting, unless the user explicitly asked for specific animals. Give THIS book its own story arc shaped by the topic: an opening with a wish or question, a small challenge or surprise in the middle, and a warm resolution. Vary how pages begin; never open several pages with the same words.';
