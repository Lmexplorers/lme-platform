# -*- coding: utf-8 -*-
"""Genererer 2 separate LME-hekleoppskrifter (runeskrift, NORGE / NORWAY, hver for seg) for
bøttehatter barn, som erstatning for den gamle samle-PDF-en."""
import base64, pathlib

BASE = pathlib.Path(__file__).parent
SRC = BASE / 'bottehatter-barn-hekle-rune'
...
VARIANTS = [
    dict(slug='norge', box=54, tmpl_box=44,
         no_word='NORGE', no_title='NORGE-BØTTEHATT<br>TIL BABY OG BARN, RUNESKRIFT',
         no_desc='Ordet «NORGE» heklet på i lesbare runestil-bokstaver, pluss et lite norsk flagg på toppen.',
         en_word='NORGE', en_title='NORGE BUCKET HAT<br>FOR BABY AND CHILD, RUNE LETTERS',
         en_desc='The word &laquo;NORGE&raquo; crocheted on in readable rune-style letters, plus a small Norwegian flag on the top.'),
    dict(slug='norway', box=48, tmpl_box=40,
         no_word='NORWAY', no_title='NORWAY-BØTTEHATT<br>TIL BABY OG BARN, RUNESKRIFT',
         no_desc='Ordet «NORWAY» heklet på i lesbare runestil-bokstaver, pluss et lite norsk flagg på toppen.',
         en_word='NORWAY', en_title='NORWAY BUCKET HAT<br>FOR BABY AND CHILD, RUNE LETTERS',
         en_desc='The word &laquo;NORWAY&raquo; crocheted on in readable rune-style letters, plus a small Norwegian flag on the top.'),
]


def build(v):
    slug = v['slug']
    out_dir = BASE / f'bottehatter-barn-hekle-rune-{slug}'
    ph_no = make_page('LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;BØTTEHATT BARN, RUNESKRIFT', 'LME HEKLE')
    pages = []

    pages.append(ph_no('''
<div class="coverimg coverrune">''' + runeword(v['no_word'], box=v['box']) + '''</div>
...
''' + card(f'<p class="center">Samme bøttehatt som NORGE-runehatt-oppskriften for voksne, heklet i fastmasker '
      'og gradert helt fra bunnen av til tjueen babyer-, barne- og ungdomsstørrelser, 50 til 170. Du hekler hele hatten '
      f'ensfarget, og hekler så bokstavene på til slutt. Denne oppskriften er komplett i seg selv, du trenger '
...
''', 1))
    ...
    pages.append(ph_no(
        ...
        f'<p>Her er ordet «{v["no_word"]}» i runestil. Hekle det på i én sammenhengende linje, sentrert midt foran, med '
        ...
        pink(v['no_word']) +
        card('<div class="stripwrap" style="transform:rotate(180deg);text-align:center">' + runeword(v['no_word'], box=v['tmpl_box']) + '</div>') +
    ...
    ph_en = make_page('LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;KIDS BUCKET HAT, RUNE LETTERS', 'LME CROCHET')
    pages = []

    pages.append(ph_en('''
<div class="coverimg coverrune">''' + runeword(v['en_word'], box=v['box']) + '''</div>
...
''' + card('<p class="center">The same bucket hat as the adult NORGE rune hat pattern, crocheted in single '
      ...
      f'whole hat in one colour, then crochet the letters on at the end. This pattern is complete on its own, '
      f'you do not need any other LME pattern to crochet it. {v["en_desc"]}</p>') + '''
...
''', 1))
    ...
    pages.append(ph_en(
        ...
        'letters and the flag onto the surface.</p>' +
    ...
    pages.append(ph_en(
        ...
        f'<p>Here is the word &laquo;{v["en_word"]}&raquo; in rune style. Crochet it on in one continuous line, centred at the front, '
        ...
        'finished hat.</p>' +
        pink(v['en_word']) +
        card('<div class="stripwrap" style="transform:rotate(180deg);text-align:center">' + runeword(v['en_word'], box=v['tmpl_box']) + '</div>') +
...
<title>{v['no_word']}-bøttehatt barn, runeskrift, LME hekleoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages_no)}</body></html>'''
    doc_en = f'''<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<title>{v['en_word']} bucket hat kids, rune letters, LME crochet pattern</title>
<style>{css}</style></head>
<body>{''.join(pages_en)}</body></html>'''

    (out_dir / 'barn_hekle_rune_no.html').write_text(doc_no, encoding='utf-8')
    (out_dir / 'barn_hekle_rune_en.html').write_text(doc_en, encoding='utf-8')
