# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Mollys rangle' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Mollys rangle, LME hekleoppskrift", "Molly's Rattle, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;MOLLYS RANGLE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;MOLLY'S RATTLE")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'MOLLYS RANGLE', "MOLLY'S RATTLE")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten lam-rangle på en trering, ca. 13 til 15 cm høy. Hodet er formet som en '
    'miniatyrutgave av Molly, med myke ører, en liten løkkemasket ulltopp, en smørgul '
    'sløyfe og volangkrage, og en rangleboks trygt gjemt inni.',
    'A little lamb rattle on a wooden ring, approx. 13 to 15 cm tall. The head is shaped '
    'like a miniature version of Molly, with soft ears, a little loop-stitch wool topknot, '
    'a butter yellow bow and ruffled collar, and a rattle capsule safely tucked inside.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM MOLLYS RANGLE", "ABOUT MOLLY'S RATTLE")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Mollys rangle hører til LME Baby Collection \"Woodland Dreams\". Dette lille lamhodet "
    'på en trering er tenkt som barnets aller første møte med Molly, laget for hender som '
    'ennå er for små til å holde selve dyret.',
    "Molly's rattle belongs to the LME Baby Collection \"Woodland Dreams\". This little lamb "
    "head on a wooden ring is meant as the child's very first meeting with Molly, made for "
...
add('garn_txt',
    "Bystrikk Merino, kremhvit (hovedfarge) og smørgult (sløyfen og kragen), samme garn som "
    "resten av Mollys oppskrift.",
    "Bystrikk Merino, cream (main colour) and butter yellow (the bow and the collar), the "
    "same yarn as the rest of Molly's pattern.")
add('garn_alt',
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Molly selv, siden hodet er lite'),
...
add('ord_lead',
    'Hele rangelen hekles med fastmasker i spiral, unntatt ulltoppen, som bruker samme '
    'løkketeknikk som Mollys egen ulltopp. Her er forkortelsene, med de vanlige amerikanske '
    'hekletermene ved siden av.',
    "The whole rattle is crocheted with single crochet in a spiral, except the wool topknot, "
    "which uses the same loop technique as Molly's own topknot. Here are the abbreviations, "
...
add('oversikt_deler', [
    ...
    ('2. Halsen og kragen', 'tube med smørgul volangkant, sys rundt treringen',
     '2. The neck and collar', 'a tube with a butter yellow ruffled edge, sewn around the '
     'wooden ring'),
    ('3. Ørene', 'to stk, myke, flate', '3. The ears', 'two, soft, flat'),
    ('4. Ulltoppen og sløyfen', 'mellom ørene', '4. The wool topknot and bow',
     'between the ears'),
])
...
add('krage_txt',
    'Før du syr hodet fast: fest smørgul tråd i halsens øverste kant (12 m). *1 fm i neste '
    ...
    'Before sewing the head on: attach butter yellow yarn at the top edge of the neck (12 '
    'sts). *1 sc in the next stitch, skip 1 stitch, 3 dc in the next stitch (a little fan)*, '
...
# ---------------------------------------------------------------- SIDE 9: ØRER, ULLTOPP OG SLØYFE
add('banner_orer', 'DEL 3: ØRENE, ULLTOPPEN OG SLØYFEN',
    'PART 3: THE EARS, THE WOOL TOPKNOT AND THE BOW')
add('pill_orer', 'ØRENE (2 STK)', 'THE EARS (MAKE 2)')
add('orer_txt',
    'Hekle to små flate sirkler i kremhvitt: 6 fm i magisk ring, økn x 6 (12). Avslutt. Sy '
    'ørene fast øverst på hodet, med litt avstand mellom, hengende lett nedover.',
    'Crochet two small flat circles in cream: 6 sc in a magic ring, inc x 6 (12). Fasten '
    'off. Sew the ears onto the top of the head, with a little space between, hanging '
    'gently downward.')
add('pill_horn', 'ULLTOPPEN OG SLØYFEN', 'THE WOOL TOPKNOT AND THE BOW')
add('horn_txt',
    'Fest kremhvit tråd øverst på hodet, mellom ørene, og hekle 6 til 8 løkkemasker rett inn '
    'på hodets overflate, tett samlet. Fest av. Hekle så en smal strimmel i smørgult: legg '
    'opp 10 lm + 1 vendemaske, 1 rad fm (10 fm). Fold strimmelen sammen til en liten '
    'sløyfeform og sy den fast oppå ulltoppen.',
    'Attach cream yarn to the top of the head, between the ears, and crochet 6 to 8 loop '
    'stitches directly onto the surface of the head, closely grouped. Fasten off. Then '
    'crochet a narrow strip in butter yellow: chain 10 + 1 turning chain, 1 row of sc (10 '
    'sc). Fold the strip together into a small bow shape and sew it on top of the wool '
    'topknot.')
...
add('montering_steg', [
    ...
    'Sy ørene fast øverst på hodet.',
    'Hekle ulltoppen mellom ørene, og sy sløyfen fast oppå den.',
    'Brodér ansiktet.',
...
add('montering_steg_en', [
    ...
    'Crochet the wool topknot between the ears, and sew the bow on top of it.',
    'Embroider the face.',
...
add('regler',
    ['Brukes alltid under tilsyn av en voksen, spesielt de første gangene, til du er trygg '
     ...
     'Løkkemaskene i ulltoppen er myke og ufarlige, men bør sjekkes jevnlig for å se at '
...
add('ferdig_txt',
    'Nå har du heklet en liten Molly-rangle. Rist den forsiktig og hør den myke lyden!',
    'Now you have crocheted a little Molly rattle. Give it a gentle shake and listen to the '
    'soft sound!')
...
add('kolleksjon_liste',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Pip's rattle", "Felix's rattle", "Luna's rattle", "Oliver's rattle",
     "Ellie's rattle", "Ellie's stroller toy",
...
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Molly som stiluttrykk-referanse, ikke selve rangelen.' if lang == 'no' else 'Photo shows Molly as a style reference, not the rattle itself.'}</p>
