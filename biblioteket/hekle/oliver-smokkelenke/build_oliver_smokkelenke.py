# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Olivers smokkelenke' (norsk + engelsk) som HTML,
...
FACE_REF = BASE / 'oliver_face_ref.jpg'
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Olivers smokkelenke, LME hekleoppskrift", "Oliver's Pacifier Clip, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;OLIVERS SMOKKELENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;OLIVER'S PACIFIER CLIP")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'OLIVERS SMOKKELENKE', "OLIVER'S PACIFIER CLIP")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten smokkelenke med et forenklet bjørnehode inspirert av Oliver, med en liten '
    'lyseblå minikrage i stedet for sløyfe, og noen myke heklede kuler med en stjerneperle. '
    'Heklet i de samme naturfargene som resten av kolleksjonen. Maks lengde er satt for å '
    'følge gjeldende sikkerhetsanbefaling for smokkeholdere.',
    "A little pacifier clip with a simplified bear head inspired by Oliver, with a little "
    "light blue mini collar instead of a bow, and a few soft crocheted balls with a star "
    "bead. Crocheted in the same natural colours as the rest of the collection. Maximum "
    "length follows current safety guidance for soother holders.")
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM OLIVERS SMOKKELENKE", "ABOUT OLIVER'S PACIFIER CLIP")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Olivers smokkelenke hører til LME Baby Collection \"Woodland Dreams\", samme skogsunivers "
    'som Oliver, den lille bjørnen. Det lille bjørnehodet på lenken er en forenklet utgave av '
    'Oliver, akkurat stor nok til å henge trygt og lekent ved siden av smokken.',
    "Oliver's pacifier clip belongs to the LME Baby Collection \"Woodland Dreams\", the same "
    "woodland world as Oliver, the little bear. The small bear head on the clip is a "
    "simplified version of Oliver, just the right size to hang safely and playfully next to "
    "the pacifier.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Minimalistisk, Montessori-inspirert og skandinavisk, i de samme naturfargene som Oliver: '
    'varmt lyst brunt og lyseblått. Rolig og enkelt, aldri overlesset.',
    'Minimalist, Montessori-inspired and Scandinavian, in the same natural colours as Oliver: '
    'warm light brown and light blue. Calm and simple, never cluttered.')
...
add('om_sikkerhet_kort',
    ...
    'er lengden på denne oppskriften bevisst kort, og hele side 11 er viet sikkerhet. Les den '
    'siden før du hekler videre.',
    "A pacifier clip is something a baby has close to their face, often alone in a pram or "
    "bed. That's why this pattern's length is deliberately short, and all of page 11 is "
...
add('garn_txt',
    "Bystrikk Merino (varmt lyst brunt og lyseblått). Samme garn som resten av Olivers "
    'oppskrift, så restene ofte strekker til.',
    "Bystrikk Merino (warm light brown and light blue). Same yarn as the rest of Oliver's "
    "pattern, so leftovers often stretch to cover this.")
add('garn_alt',
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Oliver selv, siden delene er små'),
...
add('oversikt_lead',
    'Tre typer deler hekles hver for seg og tres/sys sammen til slutt langs en kort snor '
    'eller direkte til hverandre:',
    'Three types of pieces are crocheted separately, then threaded or sewn together at the '
    'end along a short cord, or directly to each other:')
add('oversikt_deler', [
    ('1. Det lille bjørnehodet', 'forenklet Oliver, ingen sløyfe', '1. The little bear head',
     'a simplified Oliver, no bow'),
    ('2. Minikragen', 'liten lyseblå kant i stedet for sløyfe', '2. The mini collar',
     'a little light blue trim instead of a bow'),
    ('3. De heklede kulene', 'så mange du trenger for å fylle ut lengden, med én '
     'stjerneperle', '3. The crocheted balls', 'as many as you need to fill out the length, '
     'with one star bead'),
])

# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: DET LILLE BJØRNEHODET', 'PART 1: THE LITTLE BEAR HEAD')
add('hode_lead',
    'En forenklet, liten utgave av Oliver sitt hode, varmt lyst brunt, uten egen snuteflekk.',
    "A simplified, small version of Oliver's head, warm light brown, without a separate "
    "muzzle patch.")
...
add('pill_orer_mini', 'TO SMÅ, RUNDE ØRER', 'TWO SMALL, ROUND EARS')
add('orer_mini_txt',
    'Hekle to små flate sirkler i varmt lyst brunt: 5 fm i magisk ring, avslutt. Sy dem fast '
    'øverst på hodet, med litt avstand mellom, helt runde og flate, den klassiske '
    'bamseformen i miniatyr.',
    "Crochet two small flat circles in warm light brown: 5 sc in a magic ring, fasten off. "
    "Sew them onto the top of the head, with a little space between, completely round and "
    "flat, the classic teddy bear shape in miniature.")
...
add('ansikt_mini_txt',
    ...
    'deler. Se referansebildet fra Oliver for inspirasjon til uttrykket, i miniatyr.',
    'Embroider two small, round eyes in black satin stitch and a tiny oval nose below. Do '
    'not use plastic safety eyes here, since the clip must be entirely free of hard or loose '
    "parts. See the reference photo from Oliver for inspiration on the expression, in "
    "miniature.")

# ---------------------------------------------------------------- SIDE 8: MINIKRAGEN
add('banner_krage', 'DEL 2: MINIKRAGEN', 'PART 2: THE MINI COLLAR')
add('pill_krage', 'MINIKRAGEN (LYSEBLÅ)', 'THE MINI COLLAR (LIGHT BLUE)')
add('krage_txt',
    'I stedet for en sløyfe får Oliver sin smokkelenke en miniatyr av signaturfargen hans: '
    'fest lyseblå tråd rundt kanten der hodet er som smalest, nederst. *1 fm i neste maske, '
    'hopp over 1 maske, 3 stav i neste maske (en liten vifte), hopp over 1 maske*, gjenta '
    'rundt hele kanten. Fest av og gjem tråden.',
    "Instead of a bow, Oliver's pacifier clip gets a miniature of his signature colour: "
    "attach light blue yarn around the edge where the head is narrowest, at the bottom. "
...
add('krage_fest_txt',
    'Minikragen hekles direkte på hodet og trenger ingen ekstra festing, den sitter fast som '
    'en liten volangkant rundt halsen, akkurat som den store kragen på Oliver selv.',
    "The mini collar is crocheted directly onto the head and needs no extra attaching, it "
    "sits in place as a little ruffled edge around the neck, just like the full-size collar "
    "on Oliver himself.")

# ---------------------------------------------------------------- SIDE 9: KULENE
add('banner_kuler', 'DEL 3: DE HEKLEDE KULENE', 'PART 3: THE CROCHETED BALLS')
add('kuler_lead',
    'Hekle så mange kuler du trenger for å fylle ut lengden mellom klipsen og bjørnehodet, '
    'gjerne i en rytme av varmt lyst brunt og lyseblått, med treperler mellom og én '
    'stjerneformet treperle et sted i rekken. Husk: total lengde skal aldri bli mer enn '
    '22 cm.',
    'Crochet as many balls as you need to fill out the length between the clip and the bear '
    'head, for example alternating warm light brown and light blue, with wooden beads in '
...
add('lokke_txt',
    ...
    'varmt lyst brunt: legg opp 16 lm + 1 vendemaske, 1 rad fm tilbake (16 fm). Fold '
    'strimmelen på langs og sy sammen til en tynn, rund snor. Bøy snoren i en løkke og sy '
    'endene godt fast til siste kule i enden av lenken. Tre smokkeringen gjennom løkka før du '
    'fester det hele.',
    'Instead of attaching the pacifier ring directly to the last ball, crochet a short, firm '
    'strip in warm light brown: chain 16 + 1 turning chain, 1 row of sc back (16 sc). Fold '
    'the strip lengthwise and sew together into a thin, round cord. Bend the cord into a '
    'loop and sew the ends securely to the last ball at the end of the clip. Thread the '
    'pacifier ring through the loop before attaching the whole piece.')
...
add('montering_steg', [
    'Hekle minikragen rundt nederste kant av bjørnehodet.',
    ...
    'Fest bjørnehodet i den ene enden.',
...
add('montering_steg_en', [
    'Crochet the mini collar around the bottom edge of the bear head.',
    ...
    'Attach the bear head at one end.',
...
add('ferdig_txt',
    'Nå har du heklet din egen smokkelenke inspirert av Oliver. Husk å sjekke lengden en '
    'siste gang før den tas i bruk!',
    "Now you have crocheted your own pacifier clip inspired by Oliver. Remember to check the "
...
add('kolleksjon_liste',
    ['Ellie, det lille dådyret', 'Pip, det lille pinnsvinet', 'Felix, den lille reven',
     ...
     'Lunas smokkelenke', 'Ellies rangle', 'Ellies vognlenke', 'Ellies ballerinasko',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Molly's pacifier clip", "Luna's pacifier clip", "Ellie's rattle", "Ellie's stroller toy",
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="figwrap"><img src="{face_src}" style="width:98mm;border-radius:14px;box-shadow:0 3px 10px rgba(0,0,0,.18);" alt="Oliver, stiluttrykk-referanse"></div>
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Oliver som stiluttrykk-referanse, ikke selve smokkelenken.' if lang == 'no' else 'Photo shows Oliver as a style reference, not the pacifier clip itself.'}</p>
...
<div class="twocol">
  ...
  <div class="figwrap"><img src="{face_src}" alt="Oliver ansikt, referanse"><div class="figcap small">{'Referanse: Olivers ansiktsuttrykk' if lang == 'no' else "Reference: Oliver's expression"}</div></div>
</div>
''', 7))
