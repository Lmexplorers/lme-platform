# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Lunas rangle' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Lunas rangle, LME hekleoppskrift", "Luna's Rattle, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;LUNAS RANGLE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;LUNA'S RATTLE")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'LUNAS RANGLE', "LUNA'S RATTLE")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten kanin-rangle på en trering, ca. 13 til 15 cm høy. Hodet er formet som en '
    'miniatyrutgave av Luna, med to små, avlange ører som en miniatyr av hennes lange ører, '
    'en rosa sløyfe og volangkrage, og en rangleboks trygt gjemt inni.',
    'A little bunny rattle on a wooden ring, approx. 13 to 15 cm tall. The head is shaped '
    "like a miniature version of Luna, with two small, elongated ears as a miniature of her "
    'long ears, a pink bow and ruffled collar, and a rattle capsule safely tucked inside.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM LUNAS RANGLE", "ABOUT LUNA'S RATTLE")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Lunas rangle hører til LME Baby Collection \"Woodland Dreams\". Dette lille kaninhodet "
    'på en trering er tenkt som barnets aller første møte med Luna, laget for hender som '
    'ennå er for små til å holde selve dyret.',
    "Luna's rattle belongs to the LME Baby Collection \"Woodland Dreams\". This little bunny "
    "head on a wooden ring is meant as the child's very first meeting with Luna, made for "
    "hands that are still too small to hold the toy itself.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Skandinavisk og Montessori-inspirert, i de samme naturfargene som resten av kolleksjonen: '
    'varmt grått og rosa. Rund, myk form uten skarpe kanter.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection: warm grey and pink. A round, soft shape with no sharp edges.')
add('pill_funksjon', 'FUNKSJON', 'FUNCTION')
add('om_funksjon',
    'Trering gir en trygg, lett gripbar håndtak-form. Rangleboksen inni hodet lager en myk '
    'raslelyd. De avlange ørene gir litt ekstra å ta tak i for små fingre.',
    'The wooden ring gives a safe, easy-to-grip handle shape. The rattle capsule inside the '
    'head makes a soft rustling sound. The elongated ears give small fingers a little extra '
    'to hold onto.')
...
add('garn_txt',
    "Bystrikk Merino, varmt grått (hovedfarge) og rosa (sløyfen og kragen), samme garn som "
    "resten av Lunas oppskrift.",
    "Bystrikk Merino, warm grey (main colour) and pink (the bow and the collar), the same "
    "yarn as the rest of Luna's pattern.")
add('garn_alt',
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Luna selv, siden hodet er lite'),
...
add('ord_lead',
    'Hele rangelen hekles med fastmasker i spiral, unntatt ørene, sløyfen og kragen, som '
    'hekles frem og tilbake i rader. Her er forkortelsene, med de vanlige amerikanske '
    'hekletermene ved siden av.',
    "The whole rattle is crocheted with single crochet in a spiral, except the ears, bow and "
    "collar, which are crocheted back and forth in rows. Here are the abbreviations, with the "
...
add('oversikt_lead',
    ...
    'Four pieces are crocheted separately, then sewn together around the wooden ring at the '
    'end:')
add('oversikt_deler', [
    ('1. Hodet', 'rommer rangleboksen inni', '1. The head', 'holds the rattle capsule inside'),
    ('2. Halsen og kragen', 'tube med rosa volangkant, sys rundt treringen',
     '2. The neck and collar', 'a tube with a pink ruffled edge, sewn around the wooden '
     'ring'),
    ('3. Ørene', 'to stk, avlange, hengende', '3. The ears', 'two, elongated, hanging'),
    ('4. Sløyfen', 'rosa, mellom ørene', '4. The bow', 'pink, between the ears'),
])

# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: HODET', 'PART 1: THE HEAD')
add('hode_lead',
    'Hodet hekles i spiral, ovenfra og ned, i varmt grått. Rangleboksen legges inn før de '
    'siste omgangene lukkes.',
    'The head is crocheted in a spiral, from the top down, in warm grey. The rattle capsule '
    'is inserted before the last rounds are closed.')
add('hode_rows', [
...
add('krage_txt',
    'Før du syr hodet fast: fest rosa tråd i halsens øverste kant (12 m). *1 fm i neste '
    ...
    'Before sewing the head on: attach pink yarn at the top edge of the neck (12 sts). *1 sc '
    'in the next stitch, skip 1 stitch, 3 dc in the next stitch (a little fan)*, repeat all '
    'the way around the edge (6 fans in total). Fasten off and weave in the end.')
...
add('pill_orer', 'ØRENE (2 STK, AVLANGE)', 'THE EARS (MAKE 2, ELONGATED)')
add('orer_txt',
    'I stedet for runde ører får Lunas rangle en miniatyr av signaturdelen hennes: legg opp '
    '4 lm + 1 vendemaske, 4 rader fm (4 fm) i varmt grått, for hvert øre. Sy dem fast øverst '
    'på hodet, med litt avstand mellom, hengende lett nedover som en miniatyr av Lunas lange '
    'ører.',
    "Instead of round ears, Luna's rattle gets a miniature of her signature part: chain 4 + 1 "
    "turning chain, 4 rows of sc (4 sc) in warm grey, for each ear. Sew them onto the top of "
    "the head, with a little space between, hanging gently downward as a miniature of Luna's "
    "long ears.")
add('pill_sloyfe', 'SLØYFEN (ROSA)', 'THE BOW (PINK)')
add('sloyfe_txt',
    'Hekle en smal strimmel i rosa: legg opp 10 lm + 1 vendemaske, 1 rad fm (10 fm). Fold '
    'strimmelen sammen til en liten sløyfeform og sy den fast mellom ørene, øverst på hodet, '
    'Lunas eget kjennetegn i miniatyr.',
    'Crochet a narrow strip in pink: chain 10 + 1 turning chain, 1 row of sc (10 sc). Fold '
    "the strip together into a small bow shape and sew it on between the ears, at the top of "
    "the head, Luna's own signature detail in miniature.")
add('pill_ansikt', 'ANSIKTET, BRODERT', 'THE FACE, EMBROIDERED')
add('ansikt_txt',
    'Brodér to runde øyne i svart satengsting og en liten trekantet nese under. Ingen '
    ...
    'Embroider two round eyes in black satin stitch and a small triangular nose below. No '
    'plastic safety eyes here, since the rattle must be entirely free of hard or loose parts '
    'apart from the rattle capsule, which is safely hidden inside.')
...
add('montering_steg', [
    ...
    'Hekle begge ørene, og sy dem fast øverst på hodet, hengende lett nedover.',
    'Hekle sløyfen og sy den fast mellom ørene.',
    'Brodér ansiktet.',
...
add('montering_steg_en', [
    ...
    'Crochet both ears, and sew them onto the top of the head, hanging gently downward.',
    'Crochet the bow and sew it on between the ears.',
    'Embroider the face.',
...
add('ferdig_txt',
    'Nå har du heklet en liten Luna-rangle. Rist den forsiktig og hør den myke lyden!',
    'Now you have crocheted a little Luna rattle. Give it a gentle shake and listen to the '
    'soft sound!')
...
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Lunas ansikt som stiluttrykk-referanse, ikke selve rangelen.' if lang == 'no' else "Photo shows Luna's face as a style reference, not the rattle itself."}</p>
