# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Lunas vognlenke' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', 'Lunas vognlenke, LME hekleoppskrift', "Luna's Stroller Toy, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;LUNAS VOGNLENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;LUNA'S STROLLER TOY")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'LUNAS VOGNLENKE', "LUNA'S STROLLER TOY")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten vognlenke med Luna midt på i en rosa volangkrage, dinglende fra en kjede av '
    'heklede kuler, treperler, en blomst og et blad, med to trering-rangler på hver side og en '
    'tre-kuleklips i hver ende som festes rett på barnevognens bøyle. Heklet i de samme '
    'naturfargene som resten av kolleksjonen, med lengde satt bevisst kort av sikkerhetshensyn.',
    "A little stroller toy with Luna in the middle in a pink ruffled collar, hanging from a "
    "chain of crocheted balls, wooden beads, a flower and a leaf, with two wooden ring rattles "
    "on each side and a wooden ball clip at each end that attaches directly to the pram's bar. "
    "Crocheted in the same natural colours as the rest of the collection, with a length kept "
    "deliberately short for safety.")
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', 'OM LUNAS VOGNLENKE', "ABOUT LUNA'S STROLLER TOY")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Lunas vognlenke hører til LME Baby Collection \"Woodland Dreams\". Luna henger midt på "
    'kjeden, med en liten blomst og et blad fra blomsterengen hun elsker på hver side, og to '
    'trering-rangler som barnet kan gripe etter og lytte til.',
    'Luna\'s stroller toy belongs to the LME Baby Collection "Woodland Dreams". Luna hangs in '
    'the middle of the chain, with a little flower and leaf from the meadow she loves on each '
    'side, and two wooden ring rattles the child can reach for and listen to.')
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Skandinavisk og Montessori-inspirert, i de samme naturfargene som resten av kolleksjonen. '
    'En myk kjede av runde kuler og treperler, med Luna som det tydelige midtpunktet.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection. A soft chain of round balls and wooden beads, with Luna as the clear centre '
    'point.')
...
add('garn_txt',
    'Bystrikk Merino (varmt grått og rosa) og rester av kremhvitt og salviegrønt, samme '
    "garnfamilie som resten av Lunas oppskrift.",
    "Bystrikk Merino (warm grey and pink) and leftover cream and sage green, the same yarn "
    "family as the rest of Luna's pattern.")
add('garn_alt',
...
add('utstyr', [
    ...
    ('Litt polyesterfiber til fyll', 'kun til Luna-medaljongen og kulene'),
...
add('ord_lead',
    "Delene bruker fastmasker, stav og halvstav, akkurat som Lunas smokkelenke.",
    "The pieces use single, double and half double crochet, just like Luna's pacifier clip.")
...
add('ord_rows', [
    ...
    ('vm', 'turning ch', 'vendemaske: telles ikke som egen maske'),
...
add('oversikt_lead',
    ...
    'Luna i midten:',
    'Four pieces are crocheted separately and attached along a short chain of balls and wooden '
    'beads, with Luna in the middle:')
add('oversikt_deler', [
    ('1. Luna-medaljongen', 'midtdelen, med volangkrage og avlange mini-ører', '1. The Luna '
     'medallion', 'the centre piece, with a ruffled collar and elongated mini ears'),
...
# ---------------------------------------------------------------- SIDE 7: LUNA-MEDALJONGEN
add('banner_medaljong', 'DEL 1: LUNA-MEDALJONGEN', 'PART 1: THE LUNA MEDALLION')
add('medaljong_lead',
    "Midtdelen er en liten, flat utgave av Lunas hode, varmt grått, lett stoppet slik at den "
    'får litt form uten å bli tung.',
    "The centre piece is a small, flat version of Luna's head, warm grey, lightly stuffed so "
    "it gets a little shape without becoming heavy.")
add('medaljong_rows', [
...
add('medaljong_ferdig',
    'Klipp av med god tråd igjen. I stedet for runde ører får Luna-medaljongen en miniatyr av '
    'signaturdelen hennes: legg opp 4 lm + 1 vendemaske, 3 rader fm i varmt grått, for hvert '
    'øre (hekle 2 stk), og sy dem fast øverst, hengende lett nedover som en miniatyr av Lunas '
    'lange ører, og brodér et lite ansikt akkurat som på rangelen og smokkelenken.',
    'Cut, leaving a long tail. Instead of round ears, the Luna medallion gets a miniature of '
    'her signature part: chain 4 + 1 turning chain, 3 rows of sc in warm grey, for each ear '
    '(make 2), and sew them onto the top, hanging gently downward as a miniature of Luna\'s '
    "long ears, and embroider a small face just like on the rattle and pacifier clip.")
add('pill_krage', 'VOLANGKRAGEN (ROSA)', 'THE RUFFLED COLLAR (PINK)')
add('krage_txt',
    'Fest rosa tråd rundt kanten der hodet er som smalest, nederst. *1 fm i neste maske, hopp '
    'over 1 maske, 3 stav i neste maske (en liten vifte), hopp over 1 maske*, gjenta rundt hele '
    'kanten. Fest av og gjem tråden. Volangkragen hekles direkte på hodet og trenger ingen '
    'ekstra festing.',
    'Attach pink yarn around the edge where the head is narrowest, at the bottom. *1 sc in the '
    'next stitch, skip 1 stitch, 3 dc in the next stitch (a little fan), skip 1 stitch*, '
    'repeat all the way around the edge. Fasten off and weave in the end. The ruffled collar '
    'is crocheted directly onto the head and needs no extra attaching.')
...
add('blomst_txt',
    ...
    'kronblad. Avslutt med kjedemaske og klipp av med god tråd igjen. Sy en liten tre-perle '
    'fast midt i blomsten om du vil, som i referansebildet.',
    ...
    'until you have six petals. Finish with a slip stitch and cut, leaving a long tail. Sew a '
    'small wooden bead onto the middle of the flower if you like, as in the reference photo.')
...
add('rangler_lead',
    ...
    'side av Luna-medaljongen. Disse er det barnet griper etter og rasler med.',
    'Two wooden rings with a small dangling crocheted ball inside each, attached partway along '
    'the chain on each side of the Luna medallion. These are what the child reaches for and '
    'rattles.')
add('rangler_txt',
    "Kulene: hekle to kuler, samme oppskrift som på Lunas smokkelenke: 6 fm i magisk ring, "
    'økn x 6 (12), 12 fm i 2 omganger, mink x 6 (6), fyll lett, fest av med god tråd igjen.',
    "The balls: crochet two balls, the same pattern as on Luna's pacifier clip: 6 sc in a "
...
add('rangler_ferdig',
    ...
    'selve ringen godt fast tett inntil hovedkjeden på hver side av Luna, med flere runder '
    ...
    'looser. Sew the ring itself securely close against the main chain on each side of Luna, '
...
add('kuler_lead',
    ...
    'varmt grått, kremhvitt og pudderrosa, med en liten tre-mellomperle mellom hver kule, som '
    'i referansebildet. Husk: total lengde skal aldri bli mer enn 35 til 40 cm.',
    'Crochet as many balls as you need to fill out the rest of the chain, for example '
    'alternating warm grey, cream and powder pink, with a small wooden spacer bead between '
    'each ball, as in the reference photo. Remember: the total length must never exceed 35 to '
    '40 cm.')
add('kuler_rows', [
...
add('kuler_ferdig',
    ...
    'kuler avhenger av hvor langt du vil at kjeden skal bli, tell alltid opp hele lenken før du '
...
add('montering_lead',
    'Legg Luna-medaljongen, blomsten, bladet, de to trering-ranglene og alle kulene ut i den '
    'rekkefølgen du vil ha dem, før du fester noe permanent.',
    'Lay out the Luna medallion, the flower, the leaf, the two wooden ring rattles and all the '
...
add('montering_steg', [
    ...
    'Sy Luna-medaljongen fast midt på kjeden, med volangkragen ferdig heklet.',
...
add('montering_steg_en', [
    ...
    'Sew the Luna medallion onto the middle of the chain, with the ruffled collar already '
    'crocheted on.',
    'Thread the balls onto the cord with a small wooden spacer bead between each one, evenly '
    'distributed on both sides of Luna.',
    'Sew the flower and leaf on partway out on each side, close against the chain.',
    'Attach the two wooden ring rattles with the balls further out on each side, securely with '
    'whip stitching.',
...
add('foto_captions',
    ['Luna-medaljongen med krage', 'Kjeden lagt ut', 'Trering-ranglene', 'Ferdig montert '
     'lenke'],
    ['The Luna medallion with collar', 'The chain laid out', 'The wooden ring rattles',
...
add('regler',
    ['Brukes alltid under tilsyn av en voksen. Ta av vognlenken når barnet sover, eller når '
     ...
     'Ørene på medaljongen er myke og flate, uten stiv innmat, men bør sjekkes jevnlig for å '
     'se at sømmene sitter godt.',
    ...
    ['Always use under adult supervision. Remove the stroller toy when the child is sleeping, '
     ...
     'cord.',
     'The ears on the medallion are soft and flat, with no stiff filling, but should be '
     'checked regularly to make sure the seams hold.',
...
add('ferdig_txt',
    'Nå har du heklet en liten Luna-vognlenke. Husk å sjekke lengden en siste gang før den '
    'festes til vognen!',
    "Now you have crocheted a little Luna stroller toy. Remember to check the length one "
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="coverimg"><img src="{ref_src}" alt="Luna, stiluttrykk-referanse"></div>
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Luna som stiluttrykk-referanse, ikke selve vognlenken.' if lang == 'no' else 'Photo shows Luna as a style reference, not the stroller toy itself.'}</p>
