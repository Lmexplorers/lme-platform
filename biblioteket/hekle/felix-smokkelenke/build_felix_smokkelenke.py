# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Felix' smokkelenke' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Felix' smokkelenke, LME hekleoppskrift", "Felix's Pacifier Clip, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;FELIX\' SMOKKELENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;FELIX'S PACIFIER CLIP")
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM FELIX' SMOKKELENKE", "ABOUT FELIX'S PACIFIER CLIP")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Felix' smokkelenke hører til LME Baby Collection \"Woodland Dreams\", samme skogsunivers "
    'som Felix, den lille reven. Det lille revehodet på lenken er en forenklet utgave av '
    'Felix, akkurat stor nok til å henge trygt og lekent ved siden av smokken.',
    "Felix's pacifier clip belongs to the LME Baby Collection \"Woodland Dreams\", the same "
    "woodland world as Felix, the little fox. The small fox head on the clip is a simplified "
    "version of Felix, just the right size to hang safely and playfully next to the pacifier.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Minimalistisk, Montessori-inspirert og skandinavisk, i de samme naturfargene som Felix: '
    'rustoransje og kremhvitt. Rolig og enkelt, aldri overlesset.',
    'Minimalist, Montessori-inspired and Scandinavian, in the same natural colours as Felix: '
    'rust orange and cream. Calm and simple, never cluttered.')
...
add('garn_txt',
    "Bystrikk Merino (rustoransje og kremhvit). Samme garn som resten av Felix' oppskrift, så "
    'restene ofte strekker til.',
    "Bystrikk Merino (rust orange and cream). Same yarn as the rest of Felix's pattern, so "
...
# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: DET LILLE REVEHODET', 'PART 1: THE LITTLE FOX HEAD')
add('hode_lead',
    'En forenklet, liten utgave av Felix sitt hode, rustoransje, uten egen snuteflekk.',
    "A simplified, small version of Felix's head, rust orange, without a separate muzzle "
    "patch.")
...
add('orer_mini_txt',
    'Hekle to små flate sirkler i rustoransje: 5 fm i magisk ring, avslutt. Fold hver sirkel '
    'lett i to for en liten spiss, en miniatyr av Felix sine spisse ører, og sy dem fast '
    'øverst på hodet, med litt avstand mellom.',
    "Crochet two small flat circles in rust orange: 5 sc in a magic ring, fasten off. Fold "
    "each circle gently in half for a small point, a miniature of Felix's pointed ears, and "
    "sew them onto the top of the head, with a little space between.")
...
add('ansikt_mini_txt',
    ...
    'deler. Se referansebildet fra Felix for inspirasjon til uttrykket, i miniatyr.',
...
add('hale_txt',
    ...
    'i magisk ring, rustoransje (6). Økn x 6 (12). 2 omganger, 12 fm. Bytt til kremhvit, 1 '
    'omgang 12 fm. Mink x 6 (6). Fyll lett og klipp av med god tråd igjen.',
    "Instead of a bow, Felix's pacifier clip gets a miniature of his signature part: 6 sc in a "
    "magic ring, rust orange (6). Inc x 6 (12). 2 rounds, 12 sc. Switch to cream, 1 round 12 "
    "sc. Dec x 6 (6). Stuff lightly and cut, leaving a long tail.")
add('pill_hale_fest', 'PLASSERING', 'PLACEMENT')
add('hale_fest_txt',
    'Fest minihalen i enden av lenken nærmest revehodet, eller tre den inn blant de andre '
    'kulene, akkurat som du synes ser finest ut.',
...
add('ferdig_txt',
    'Nå har du heklet din egen smokkelenke inspirert av Felix. Husk å sjekke lengden en siste '
    'gang før den tas i bruk!',
    "Now you have crocheted your own pacifier clip inspired by Felix. Remember to check the "
...
add('kolleksjon_liste',
    ['Ellie, det lille dådyret', 'Pip, det lille pinnsvinet', 'Felix, den lille reven',
     ...
     'Ellies smokkelenke', 'Pips smokkelenke', 'Ellies rangle', 'Ellies vognlenke',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Ellie's pacifier clip", "Pip's pacifier clip", "Ellie's rattle", "Ellie's stroller toy",
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="figwrap"><img src="{face_src}" style="width:98mm;border-radius:14px;box-shadow:0 3px 10px rgba(0,0,0,.18);" alt="Felix, stiluttrykk-referanse"></div>
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Felix som stiluttrykk-referanse, ikke selve smokkelenken.' if lang == 'no' else 'Photo shows Felix as a style reference, not the pacifier clip itself.'}</p>
...
<div class="twocol">
  ...
  <div class="figwrap"><img src="{face_src}" alt="Felix ansikt, referanse"><div class="figcap small">{'Referanse: Felix' + chr(39) + ' ansiktsuttrykk' if lang == 'no' else "Reference: Felix's expression"}</div></div>
</div>
''', 7))
