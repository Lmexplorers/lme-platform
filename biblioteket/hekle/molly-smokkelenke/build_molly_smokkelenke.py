# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Mollys smokkelenke' (norsk + engelsk) som HTML,
...
FACE_REF = BASE / 'molly_face_ref.jpg'
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Mollys smokkelenke, LME hekleoppskrift", "Molly's Pacifier Clip, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;MOLLYS SMOKKELENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;MOLLY'S PACIFIER CLIP")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'MOLLYS SMOKKELENKE', "MOLLY'S PACIFIER CLIP")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten smokkelenke med et forenklet lamhode inspirert av Molly, med en smørgul '
    'volangkrage rundt halsen, en liten krøllete løkkemasket topp og en smørgul sløyfe, og '
    'noen myke heklede kuler med en stjerneperle. Heklet i de samme naturfargene som resten '
    'av kolleksjonen. Maks lengde er satt for å følge gjeldende sikkerhetsanbefaling for '
    'smokkeholdere.',
    "A little pacifier clip with a simplified lamb head inspired by Molly, with a butter "
    "yellow ruffled collar around the neck, a little curly loop-stitch topknot and a butter "
    "yellow bow, and a few soft crocheted balls with a star bead. Crocheted in the same "
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM MOLLYS SMOKKELENKE", "ABOUT MOLLY'S PACIFIER CLIP")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Mollys smokkelenke hører til LME Baby Collection \"Woodland Dreams\", samme skogsunivers "
    'som Molly, det lille lammet. Det lille lamhodet på lenken er en forenklet utgave av '
    'Molly, akkurat stor nok til å henge trygt og lekent ved siden av smokken.',
    "Molly's pacifier clip belongs to the LME Baby Collection \"Woodland Dreams\", the same "
    "woodland world as Molly, the little lamb. The small lamb head on the clip is a "
    "simplified version of Molly, just the right size to hang safely and playfully next to "
    "the pacifier.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Minimalistisk, Montessori-inspirert og skandinavisk, i de samme naturfargene som Molly: '
    'kremhvitt og smørgult. Rolig og enkelt, aldri overlesset.',
    'Minimalist, Montessori-inspired and Scandinavian, in the same natural colours as Molly: '
    'cream and butter yellow. Calm and simple, never cluttered.')
...
add('garn_txt',
    "Bystrikk Merino (kremhvit og smørgul). Samme garn som resten av Mollys oppskrift, så "
    'restene ofte strekker til.',
    "Bystrikk Merino (cream and butter yellow). Same yarn as the rest of Molly's pattern, so "
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Molly selv, siden delene er små'),
...
add('ord_lead',
    'Denne oppskriften bruker samme løkkemaske-teknikk som Mollys ulltopp, i miniatyr. Her er '
    'alle forkortelsene, med de vanlige amerikanske hekletermene ved siden av.',
    "This pattern uses the same loop-stitch technique as Molly's wool topknot, in miniature. "
    "Here are all the abbreviations, with the common US crochet terms alongside.")
...
add('oversikt_lead',
    'Fire typer deler hekles hver for seg og tres/sys sammen til slutt langs en kort snor '
    'eller direkte til hverandre:',
    'Four types of pieces are crocheted separately, then threaded or sewn together at the '
    'end along a short cord, or directly to each other:')
add('oversikt_deler', [
    ('1. Det lille lamhodet', 'forenklet Molly med mini-ulltopp', '1. The little lamb head',
     'a simplified Molly with a mini wool topknot'),
    ('2. Volangkragen', 'smørgul, rundt halsen', '2. The ruffled collar', 'butter yellow, '
     'around the neck'),
    ('3. Sløyfen', 'smørgul, mellom ørene', '3. The bow', 'butter yellow, between the ears'),
    ('4. De heklede kulene', 'så mange du trenger for å fylle ut lengden, med én '
     'stjerneperle', '4. The crocheted balls', 'as many as you need to fill out the length, '
     'with one star bead'),
])

# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: DET LILLE LAMHODET', 'PART 1: THE LITTLE LAMB HEAD')
add('hode_lead',
    'En forenklet, liten utgave av Mollys hode, kremhvit.',
    "A simplified, small version of Molly's head, cream.")
...
add('orer_mini_txt',
    'Hekle to små flate sirkler i kremhvitt: 5 fm i magisk ring, avslutt. Sy dem fast øverst '
    'på hodet, med litt avstand mellom, som en forenklet miniatyr av Mollys lange, slappe '
    'ører.',
    "Crochet two small flat circles in cream: 5 sc in a magic ring, fasten off. Sew them onto "
    "the top of the head, with a little space between, as a simplified miniature of Molly's "
    "long, floppy ears.")
add('pill_ulltopp_mini', 'DEN LILLE ULLTOPPEN', 'THE LITTLE WOOL TOPKNOT')
add('ulltopp_mini_txt',
    'Fest kremhvit tråd øverst på hodet, mellom ørene, og hekle 6 til 8 løkkemasker rett inn '
    'på hodets overflate, tett samlet, akkurat samme teknikk som Mollys egen ulltopp, bare '
    'mye mindre. Fest av og gjem tråden.',
    "Attach cream yarn to the top of the head, between the ears, and crochet 6 to 8 loop "
    "stitches directly onto the surface of the head, closely grouped, the exact same "
    "technique as Molly's own wool topknot, just much smaller. Fasten off and weave in the "
    "end.")
add('pill_ansikt_mini', 'ANSIKTET, BRODERT', 'THE FACE, EMBROIDERED')
add('ansikt_mini_txt',
    'Brodér to små, runde øyne i svart satengsting og en liten oval nese under. Ikke bruk '
    'sikkerhetsøyne med plastdeler her, siden lenken skal være helt uten harde eller løse '
    'deler. Se referansebildet fra Molly for inspirasjon til uttrykket, i miniatyr.',
    'Embroider two small, round eyes in black satin stitch and a tiny oval nose below. Do not '
    'use plastic safety eyes here, since the clip must be entirely free of hard or loose '
    "parts. See the reference photo from Molly for inspiration on the expression, in "
    "miniature.")

# ---------------------------------------------------------------- SIDE 8: VOLANGKRAGEN
add('banner_krage', 'DEL 2: VOLANGKRAGEN', 'PART 2: THE RUFFLED COLLAR')
add('pill_krage', 'VOLANGKRAGEN (SMØRGUL)', 'THE RUFFLED COLLAR (BUTTER YELLOW)')
add('krage_txt',
    'Fest smørgul tråd rundt kanten der hodet er som smalest, nederst. *1 fm i neste maske, '
    'hopp over 1 maske, 3 stav i neste maske (en liten vifte), hopp over 1 maske*, gjenta '
    'rundt hele kanten. Fest av og gjem tråden.',
    'Attach butter yellow yarn around the edge where the head is narrowest, at the bottom. '
...
# ---------------------------------------------------------------- SIDE 9: SLØYFEN
add('banner_sloyfe', 'DEL 3: SLØYFEN', 'PART 3: THE BOW')
add('pill_sloyfe', 'SLØYFEN (SMØRGUL)', 'THE BOW (BUTTER YELLOW)')
add('sloyfe_txt',
    'Hekle en smal strimmel i smørgult: legg opp 10 lm + 1 vendemaske, 1 rad fm (10 fm). Fold '
    'strimmelen sammen til en liten sløyfeform og sy et lite stykke garn stramt rundt midten '
    'flere ganger.',
    'Crochet a narrow strip in butter yellow: chain 10 + 1 turning chain, 1 row of sc (10 '
    'sc). Fold the strip together into a small bow shape and wrap a short length of yarn '
    'tightly around the centre a few times.')
add('pill_sloyfe_fest', 'FEST SLØYFEN TIL HODET', 'ATTACH THE BOW TO THE HEAD')
add('sloyfe_fest_txt',
    'Sy sløyfen fast permanent og flatt mellom ørene, oppå den lille ulltoppen, akkurat som i '
    'referansebildet.',
    'Sew the bow on permanently and flat, between the ears, on top of the little wool '
    'topknot, just like in the reference photo.')

# ---------------------------------------------------------------- SIDE 10: KULENE
add('banner_kuler', 'DEL 4: DE HEKLEDE KULENE', 'PART 4: THE CROCHETED BALLS')
add('kuler_lead',
    'Hekle så mange kuler du trenger for å fylle ut lengden mellom klipsen og lamhodet, '
    'gjerne i en rytme av kremhvitt og smørgult, med treperler mellom og én stjerneformet '
    'treperle et sted i rekken. Husk: total lengde skal aldri bli mer enn 22 cm.',
    'Crochet as many balls as you need to fill out the length between the clip and the lamb '
    'head, for example alternating cream and butter yellow, with wooden beads in between and '
...
add('montering_lead',
    'Legg alle delene ut i den rekkefølgen du vil ha dem, og mål hele lenken strukket ut før '
    'du syr eller knyter noe fast.',
...
add('montering_steg', [
    'Hekle ulltoppen på lamhodet, hekle volangkragen rundt nederste kant, og sy sløyfen fast '
    'mellom ørene, oppå ulltoppen.',
...
add('montering_steg_en', [
    'Crochet the wool topknot onto the lamb head, crochet the ruffled collar around the '
    'bottom edge, and sew the bow on between the ears, on top of the topknot.',
    'Thread or sew the balls and the star bead (and any other beads) in the order you want, '
    'on a short, strong cotton cord, or sew them directly together with tight running '
    'stitch between each piece.',
    'Attach the lamb head at one end.',
    'Crochet the cord loop and sew it securely to the last ball at the other end, thread the '
    'pacifier ring through the loop.',
    'Measure the whole clip fully stretched out. If it is longer than 22 cm, remove a ball '
    'or two.',
    'Sew the finished clip securely onto the wooden ball clip, with plenty of tight '
    'stitches.',
...
add('foto_captions',
    ['Lamhodet ferdig', 'Ulltoppen og sløyfen', 'Kulene på rekke', 'Ferdig montert lenke'],
    ['The finished lamb head', 'The wool topknot and bow', 'The balls in a row',
...
add('lengde_txt',
    ...
    'This pattern follows the principle in the European standard EN 12586 for soother '
    "holders: a simple pacifier clip (clip + strap/chain with no other attachment features) "
    "must not be longer than 220 mm (22 cm) fully stretched out. The purpose is to make it "
    "impossible for the strap to form a loop around the child's neck. Never crochet a clip "
    "longer than this, regardless of the child's age.")
...
add('regler', [
    ...
    'Løkkemaskene i ulltoppen er myke og ufarlige, men sjekk jevnlig at ingen løkke har blitt '
    'så løs at et lite barn kan få en finger fast i den.',
...
add('regler_en', [
    ...
    'The loop stitches in the topknot are soft and harmless, but check regularly that no '
    "loop has become so loose that a small child's finger could get caught in it.",
...
add('ferdig_txt',
    'Nå har du heklet din egen smokkelenke inspirert av Molly. Husk å sjekke lengden en siste '
    'gang før den tas i bruk!',
    "Now you have crocheted your own pacifier clip inspired by Molly. Remember to check the "
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="figwrap"><img src="{face_src}" style="width:98mm;border-radius:14px;box-shadow:0 3px 10px rgba(0,0,0,.18);" alt="Molly, stiluttrykk-referanse"></div>
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Molly som stiluttrykk-referanse, ikke selve smokkelenken.' if lang == 'no' else 'Photo shows Molly as a style reference, not the pacifier clip itself.'}</p>
...
{sagep(t('pill_ulltopp_mini'))}
{card('<p>' + t('ulltopp_mini_txt') + '</p>')}
''', 7))
...
<div class="twocol">
  ...
  <div class="figwrap"><img src="{face_src}" alt="Molly ansikt, referanse"><div class="figcap small">{'Referanse: Mollys ansiktsuttrykk' if lang == 'no' else "Reference: Molly's expression"}</div></div>
</div>
...
''', 9))
    ...
    pages.append(pg(f'''
{sagep(t('pill_sloyfe_fest'))}
{cme(t('sloyfe_fest_txt'))}
