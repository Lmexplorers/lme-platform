# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Mollys aktivitetsleke' (norsk + engelsk) som HTML,
...
REF = BASE / 'molly_aktivitetsleke_real.jpg'
...
alle_sider_src = img_src('molly_alle_sider.jpg')
speil_src = img_src('molly_speil.jpg')
form_src = img_src('molly_form.jpg')
lomme_src = img_src('molly_lomme.jpg')
knitre_src = img_src('molly_knitre.jpg')
...
add('caption_lomme', 'Lomme-siden med miniatyr-Molly, stiluttrykk-referanse.',
    'The pocket side with miniature Molly, style reference.')
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', 'Mollys aktivitetsleke, LME hekleoppskrift', "Molly's Activity Toy, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;MOLLYS AKTIVITETSLEKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;MOLLY'S ACTIVITY TOY")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'MOLLYS AKTIVITETSLEKE', "MOLLY'S ACTIVITY TOY")
...
add('cover_desc',
    ...
    'seks sidene: Molly med smørgul volangkrage, ulltopp og sløyfe, et babysikkert speil i en '
    'gulkantet blomsterramme, fire enkle former, Molly som titter opp av en liten lomme, og en '
    'knitreside med en liten sky som knitrer når man klemmer den. To hjørneringer i tre med '
    'dinglende perledusker og en perlekledd håndtakssnor på toppen fullfører kuben.',
    'A soft activity cube, approx. 11 to 12 cm per side, full of things to explore on each of '
    'the six sides: Molly with a butter yellow ruffled collar, wool topknot and bow, a '
    'baby-safe mirror in a yellow-edged flower frame, four simple shapes, Molly peeking out of '
    'a little pocket, and a crinkle side with a little cloud that crinkles when you squeeze it. '
    'Two wooden corner rings with dangling bead tassels and a bead-strung handle cord along the '
    'top complete the cube.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', 'OM MOLLYS AKTIVITETSLEKE', "ABOUT MOLLY'S ACTIVITY TOY")
add('pill_historien', 'DEL AV MOLLYS VERDEN', "PART OF MOLLY'S WORLD")
add('om_historien',
    'Mollys aktivitetsleke hører til LME Baby Collection "Woodland Dreams". Kuben samler hele '
    'Mollys verden på seks små sider: Molly selv med ulltoppen og den smørgule sløyfen sin, '
    'speilet der barnet møter sitt eget speilbilde, formene fra blomsterengens ting, lommen der '
    'Molly titter frem, og skyen som knitrer på knitresiden.',
    'Molly\'s activity toy belongs to the LME Baby Collection "Woodland Dreams". The cube '
    'gathers Molly\'s whole world across six small sides: Molly herself with her wool topknot '
    'and butter yellow bow, the mirror where the child meets their own reflection, shapes from '
    'the meadow, the pocket where Molly peeks out, and the cloud that crinkles on the crinkle '
    'side.')
...
add('om_stil',
    ...
    'med Mollys signaturfarge smørgult på krage, kanter og håndtakssnor. Hver side har sin egen '
    'oppgave å utforske, fra å kjenne igjen former til å høre en liten sky knitre, akkurat den '
    'typen sanselige, hands-on lek Montessori-filosofien bygger på.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection, with Molly\'s signature butter yellow on the collar, edging and handle cord. '
...
add('garn_txt',
    'Bystrikk Merino i kremhvitt (kubens sider) og lyst brunt (kantfarge), pluss rester av '
    'smørgult (Mollys krage, sløyfe og speilrammen), pudderrosa, salviegrønt og lys himmelblå, '
    'samme garnfamilie som resten av Molly-kolleksjonen.',
    'Bystrikk Merino in cream (the cube sides) and light brown (the edging colour), plus '
    'leftover butter yellow (Molly\'s collar, bow and the mirror frame), powder pink, sage '
    'green and light sky blue, the same yarn family as the rest of the Molly collection.')
add('garn_alt',
...
add('utstyr', [
    ...
    ('Polyesterfiber til fyll', 'til hjørner/kanter og til Molly-motivene'),
    ...
    ('To trerelaterte teetheringer', 'umalt/BPA-fri, CE-merket, beregnet for barn, til '
     'hjørnene'),
    ('Runde treperler i noen få størrelser', 'BPA-frie, til topphåndtaket, hjørneduskene og '
     'ulltoppen'),
...
add('vanskelig_txt',
    ...
    'du har heklet minst én av de andre Molly-oppskriftene først.',
    ...
    'A good pattern once you have crocheted at least one of the other Molly patterns first.')
...
add('ord_rows', [
    ...
    ('løkkm', 'loop st', 'løkkemaske: brukes til Mollys ulltopp på Molly-siden'),
...
add('oversikt_deler', [
    ...
    ('2. Molly-siden', 'forsiden, med ulltopp, sløyfe og volangkrage', '2. The Molly side',
     'the front, with a wool topknot, a bow and a ruffled collar'),
    ('3. Speil-siden', 'babysikkert speil i gulkantet blomsterramme', '3. The mirror side',
     'baby-safe mirror in a yellow-edged flower frame'),
    ('4. Form-siden', 'hjerte, stjerne, sirkel og trekant', '4. The shapes side',
     'heart, star, circle and triangle'),
    ('5. Lomme-siden', 'miniatyr-Molly titter opp av en lomme', '5. The pocket side',
     'a miniature Molly peeking out of a pocket'),
    ...
    ('9. Topphåndtaket', 'perlekledd snor langs den øverste sømmen', '9. The top handle',
     'a bead-strung cord along the top seam'),
...
# ---------------------------------------------------------------- SIDE 8: MOLLY-SIDEN
add('banner_molly', 'DEL 2: MOLLY-SIDEN', 'PART 2: THE MOLLY SIDE')
add('molly_lead',
    'Forsiden av kuben er Molly selv, med en tett løkkemasket ulltopp, en smørgul sløyfe og en '
    'smørgul volangkrage, samme teknikk som på rangelen og vognlenkens medaljong.',
    "The front of the cube is Molly herself, with a dense loop-stitch wool topknot, a butter "
    "yellow bow and a butter yellow ruffled collar, the same technique as on the rattle and the "
    "stroller toy's medallion.")
add('molly_rows', [
...
add('molly_ferdig',
    'Klipp av med god tråd igjen. Sy to små, myke ører i ett lag (5 fm i magisk ring, avslutt, '
    'hekle 2 stk) på hver sin side øverst, og brodér et lite ansikt.',
    'Cut, leaving a long tail. Sew on two small, soft single-layer ears (5 sc in a magic ring, '
    'fasten off, make 2) on either side at the top, and embroider a small face.')
add('molly_ulltopp',
    'Ulltoppen: fest kremhvit tråd øverst på hodet, mellom ørene, og hekle 6 til 8 løkkemasker '
    'rett inn på overflaten, tett i tett, slik at det danner en liten, tett ulltopp.',
    'The wool topknot: attach cream yarn to the top of the head, between the ears, and crochet '
    '6 to 8 loop stitches directly onto the surface, close together, so it forms a small, '
    'dense wool topknot.')
add('molly_sloyfe',
    'Sløyfen: hekle en smal strimmel i smørgult, legg opp 16 lm + 1 vendemaske, 1 rad fm (16 '
    'fm). Fold strimmelen til en sløyfeform og sy stramt sammen på midten flere ganger, som '
    'beskrevet i Mollys egen hovedoppskrift. Sy sløyfen fast permanent og flatt, oppå '
    'ulltoppen. I motsetning til på selve Molly-dyret skal sløyfen her IKKE være avtakbar, '
    'siden den sys rett på et flatt panel.',
    'The bow: crochet a narrow strip in butter yellow, chain 16 + 1 turning chain, 1 row of sc '
    '(16 sc). Fold the strip into a bow shape and sew it tightly together at the centre several '
    'times, as described in Molly\'s own main pattern. Sew the bow on permanently and flat, on '
    'top of the wool topknot. Unlike on the Molly toy itself, this bow should NOT be '
    'removable, since it is sewn directly onto a flat panel.')
add('molly_krage',
    'Volangkragen: fest smørgul tråd i nedre kant av hodet. *1 fm i neste maske, hopp over 1 '
    'maske, 5 stav i neste maske (en liten vifte), hopp over 1 maske*, gjenta hele veien rundt '
    'underkanten. Fest av. Sy til slutt Mollys hode godt fast midt på panelet.',
    'The ruffled collar: attach butter yellow yarn at the bottom edge of the head. *1 sc in the '
    'next stitch, skip 1 stitch, 5 dc in the next stitch (a little fan), skip 1 stitch*, repeat '
    'all the way around the bottom edge. Fasten off. Finally, sew Molly\'s head securely onto '
...
add('speil_txt',
    ...
    'of the pocket.')
add('speil_ramme',
    'Blomsterrammen: fest smørgul tråd direkte i ytterkanten av den innsydde speilsirkelen, og '
    ...
    'åtte kronblad rundt hele speilet, som gir den gulkantede rammen du ser i referansebildet. '
    'Fest av og gjem tråden. Sy speilet med blomsterrammen godt fast midt på panelet.',
    'The flower frame: attach butter yellow yarn directly into the outer edge of the sewn-in '
    ...
    'sl st*, repeat until you have eight petals all the way around the mirror, giving the '
    'yellow-edged frame you see in the reference photo. Fasten off and weave in the end. Sew '
    'the mirror with its flower frame securely onto the middle of the panel.')
add('speil_note',
...
add('pill_stjerne_form', 'STJERNEN (PUDDERROSA)', 'THE STAR (POWDER PINK)')
add('stjerne_form_txt',
    ...
    'Brodér gjerne et lite, fredelig ansikt med lukkede øyne på stjernen, som i referansebildet, '
    'før du syr den fast.',
    ...
    'press flat and sew on. Feel free to embroider a small, peaceful face with closed eyes onto '
    'the star, as in the reference photo, before sewing it on.')
...
add('lomme_lead',
    'En liten miniatyr-Molly titter opp av en lomme nederst på panelet, ved siden av en liten '
    'brodert blomst, en liten tittelek-overraskelse.',
    'A tiny miniature Molly peeks up out of a pocket at the bottom of the panel, next to a '
    'little embroidered flower, a little peekaboo surprise.')
add('lomme_txt',
    'Lommen: legg opp 12 lm + 1 vendemaske i smørgult. Rad 1: 12 fm. Gjenta rad 1 i 8 rader '
    ...
    'opp.',
    'The pocket: chain 12 + 1 turning chain in butter yellow. Row 1: 12 sc. Repeat row 1 for 8 '
...
add('lomme_molly',
    'Miniatyr-Molly: 6 fm i magisk ring, kremhvitt (6), økn x 6 (12), (1 fm, økn) x 6 (18), 1 '
    'omgang rett (18), mink x 6 (9), fyll svært lett. Fest av med god tråd igjen. Sy to '
    'bittesmå ører i ett lag og brodér et lite ansikt, akkurat som på Molly-siden.',
    'Miniature Molly: 6 sc in a magic ring, cream (6), inc x 6 (12), (1 sc, inc) x 6 (18), 1 '
    'round straight (18), dec x 6 (9), stuff very lightly. Fasten off, leaving a long tail. Sew '
    'on two tiny single-layer ears and embroider a small face, just like on the Molly side.')
add('lomme_montering',
    'Sy miniatyr-Molly godt fast rett bak lommeåpningen, litt høyere enn selve lommen, slik at '
    'bare hodet stikker opp av lommen, som om hun titter frem. Brodér gjerne en liten blomst '
    'ved siden av lommen, som i referansebildet. Molly skal sys helt fast til panelet, ikke '
    'bare ligge løst nedi lommen.',
    'Sew the miniature Molly securely just behind the pocket opening, a little higher than the '
    'pocket itself, so that only the head pokes up out of the pocket, as if she is peeking out. '
    'Feel free to embroider a small flower next to the pocket, as in the reference photo. Molly '
    'must be sewn completely onto the panel, not just placed loosely inside the pocket.')
...
add('flette_hull',
    ...
    'metode som skyen på Mollys vognlenke: (a) 6 fm i magisk ring. (b) 6 fm i magisk ring, økn x '
    ...
    'baksiden av puten. Brodér gjerne et lite, fredelig ansikt med lukkede øyne på fremsiden av '
    'skyen, som i referansebildet.',
    'The cloud, front and back: crochet two matching sets of three overlapping circles in '
    'cream, the same method as the cloud on Molly\'s stroller toy: (a) 6 sc in a magic ring. '
    ...
    'to embroider a small, peaceful face with closed eyes onto the front of the cloud, as in '
    'the reference photo.')
add('flette_snor',
...
add('sammensying_steg', [
    'Legg de seks ferdige panelene ut i den rekkefølgen du vil ha dem: Molly-siden foran, '
    ...
    'Lay out the six finished panels in the order you want them: the Molly side at the front, '
...
add('handtak_txt',
    ...
    'himmelblå og smørgult, akkurat som på Mollys vognlenke) på en kort, sterk bomullssnor, i '
    'den rekkefølgen du liker. Sy begge endene av snoren godt fast langs den øverste sømmen, '
    'mellom Molly-siden og lomme-siden, slik at perlerekken danner en liten bue over toppen, '
    'som et håndtak.',
    "Thread a selection of round wooden beads and yarn-covered balls (powder pink, sage green, "
    "cream, light sky blue and butter yellow, just like on Molly's stroller toy) onto a short, "
    "sturdy cotton cord, in whatever order you like. Sew both ends of the cord securely along "
    "the top seam, between the Molly side and the pocket side, so the row of beads forms a "
    "small arch over the top, like a handle.")
add('handtak_note',
...
add('hjorner_txt',
    ...
    'Molly-siden (sømmen mot speil-siden på den ene siden, sømmen mot form-siden på den andre), '
    ...
    "Sew a wooden teething ring securely into each of the two vertical seams flanking the Molly "
...
add('dusker_txt',
    'Lag to korte perledusker, hver med to dinglende kuler: tre 2 til 3 kuler/perler (for '
    'eksempel én garnkledd kule, én treperle, én garnkledd kule i en annen farge) på en kort '
    'bomullssnor, maks 6 til 8 cm lang, og sy det faste festepunktet godt fast like ved hver '
    'hjørnering, som i referansebildet. Ingen løkke skal være løs eller lang nok til å kunne '
    'strekkes ut over 8 cm.',
    'Make two short bead tassels, each with two dangling balls: thread 2 to 3 balls/beads (for '
    'example one yarn-covered ball, one wooden bead, one yarn-covered ball in a different '
    'colour) onto a short cotton cord, no more than 6 to 8 cm long, and sew the fixed '
    'attachment point securely right next to each corner ring, as in the reference photo. No '
    'loop should be loose or long enough to stretch out beyond 8 cm.')
...
add('montering_steg', [
    ...
    'Hekle Molly-siden, speil-siden, form-siden, lomme-siden og knitre-siden, hver på sitt eget '
    ...
    'Fest de to hjørneringene og de to perleduskene i sømmene som flankerer Molly-siden.',
...
add('montering_steg_en', [
    ...
    'Crochet the Molly side, the mirror side, the shapes side, the pocket side and the crinkle '
    ...
    'Attach the two corner rings and the two bead tassels into the seams flanking the Molly '
    'side.',
...
add('smadeler_txt',
    ...
    'annen oppskrift i Molly-kolleksjonen. Hvert eneste motiv, hver ring og hver perle skal sys '
    ...
    'other pattern in the Molly collection. Every single motif, every ring and every bead must '
...
add('regler',
    ['Brukes alltid under tilsyn av en voksen, spesielt de første gangene, til du er trygg på '
     ...
     'i en åpen søm.',
     'Løkkemaskene i Mollys ulltopp er myke og ufarlige, men bør sjekkes jevnlig for å se at '
     'ingen løkke har blitt så løs at et lite barn kan få en finger fast i den.',
    ...
    ['Always use under adult supervision, especially the first few times, until you are '
     ...
     'The loop stitches in Molly\'s wool topknot are soft and harmless, but should be checked '
...
add('ferdig_txt',
    'Nå har du heklet den mest detaljerte oppskriften i hele Molly-kolleksjonen. Godt jobbet, '
    'og god utforskning!',
    'Now you have crocheted the most detailed pattern in the whole Molly collection. Well '
...
add('kolleksjon_liste',
    ['Molly, det lille lammet', 'Mollys smokkelenke', 'Mollys rangle', 'Mollys vognlenke',
     'Mollys ballerinasko med sløyfe'],
    ['Molly, the little lamb', "Molly's pacifier clip", "Molly's rattle",
     "Molly's stroller toy", "Molly's ballerina shoes with a bow"])
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="coverimg"><img src="{ref_src}" alt="Mollys aktivitetsleke, stiluttrykk-referanse"></div>
...
<div class="figwrap"><img src="{alle_sider_src}" alt="Mollys aktivitetsleke, alle sidene" style="width:100mm"><div class="figcap">{t('caption_alle_sider')}</div></div>
''', 6))
...
''', 7))
    ...
    molly_rows = T['molly_rows']['no'] if lang == 'no' else T['molly_rows_en']['no']
    pages.append(pg(f'''
{banner(t('banner_molly'))}
<p>{t('molly_lead')}</p>
{card(otab(molly_rows, head3[lang]))}
{cme(t('molly_ferdig'))}
{card('<p>' + t('molly_ulltopp') + '</p>')}
{card('<p>' + t('molly_sloyfe') + '</p>')}
{card('<p>' + t('molly_krage') + '</p>')}
''', 8))
