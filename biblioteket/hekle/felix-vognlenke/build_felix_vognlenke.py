# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Felix' vognlenke' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Felix' vognlenke, LME hekleoppskrift", "Felix's Stroller Toy, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;FELIX\' VOGNLENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;FELIX'S STROLLER TOY")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', "FELIX' VOGNLENKE", "FELIX'S STROLLER TOY")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten vognlenke med Felix midt på i en salviegrønn volangkrage, dinglende fra en kjede '
    ...
    "A little stroller toy with Felix in the middle in a sage green ruffled collar, hanging "
    "from a chain of crocheted balls, wooden beads, a flower and a leaf, with two wooden ring "
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM FELIX' VOGNLENKE", "ABOUT FELIX'S STROLLER TOY")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Felix' vognlenke hører til LME Baby Collection \"Woodland Dreams\". Felix henger midt på "
    'kjeden, med en liten blomst og et blad fra skogkanten han elsker på hver side, og to '
    'trering-rangler som barnet kan gripe etter og lytte til.',
    "Felix's stroller toy belongs to the LME Baby Collection \"Woodland Dreams\". Felix hangs "
    "in the middle of the chain, with a little flower and leaf from the forest edge he loves "
    "on each side, and two wooden ring rattles the child can reach for and listen to.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Skandinavisk og Montessori-inspirert, i de samme naturfargene som resten av kolleksjonen. '
    'En myk kjede av runde kuler og treperler, med Felix som det tydelige midtpunktet.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection. A soft chain of round balls and wooden beads, with Felix as the clear centre '
    'point.')
...
add('garn_txt',
    'Bystrikk Merino (rustoransje og kremhvitt) og rester av pudderrosa og salviegrønt, samme '
    "garnfamilie som resten av Felix' oppskrift.",
    "Bystrikk Merino (rust orange and cream) and leftover powder pink and sage green, the same "
    "yarn family as the rest of Felix's pattern.")
add('garn_alt',
...
add('utstyr', [
    ...
    ('Litt polyesterfiber til fyll', 'kun til Felix-medaljongen, minihalen og kulene'),
...
add('ord_lead',
    "Delene bruker fastmasker, stav og halvstav, akkurat som Felix' smokkelenke.",
    "The pieces use single, double and half double crochet, just like Felix's pacifier clip.")
...
add('oversikt_lead',
    ...
    'beads, with Felix in the middle:')
add('oversikt_deler', [
    ('1. Felix-medaljongen', 'midtdelen, med volangkrage og minihale', '1. The Felix medallion',
     'the centre piece, with a ruffled collar and mini tail'),
...
# ---------------------------------------------------------------- SIDE 7: FELIX-MEDALJONGEN
add('banner_medaljong', 'DEL 1: FELIX-MEDALJONGEN', 'PART 1: THE FELIX MEDALLION')
add('medaljong_lead',
    "Midtdelen er en liten, flat utgave av Felix' hode, rustoransje, lett stoppet slik at den "
    'får litt form uten å bli tung.',
    "The centre piece is a small, flat version of Felix's head, rust orange, lightly stuffed "
    "so it gets a little shape without becoming heavy.")
add('medaljong_rows', [
...
add('medaljong_ferdig',
    'Klipp av med god tråd igjen. Sy to små todelte og foldede ører (rustoransje utenpå, '
    'kremhvitt inni, samme teknikk som på Felix selv) øverst, hekle en mini tofarget hale '
    '(6 fm i magisk ring rustoransje, økn x 6, 2 omganger 12 fm, bytt til kremhvitt, 1 omgang '
    '12 fm, mink x 6, fyll lett) og sy den til bakhodet, rett under ørene, og brodér et lite '
    'ansikt akkurat som på rangelen og smokkelenken.',
    'Cut, leaving a long tail. Sew on two small folded two-layer ears (rust orange outside, '
    "cream inside, the same technique as on Felix himself) on top, crochet a mini two-tone "
    'tail (6 sc in a magic ring rust orange, inc x 6, 2 rounds of 12 sc, switch to cream, 1 '
    'round of 12 sc, dec x 6, stuff lightly) and sew it onto the back of the head, right below '
    "the ears, and embroider a small face just like on the rattle and pacifier clip.")
...
add('rangler_lead',
    ...
    'side av Felix-medaljongen. Disse er det barnet griper etter og rasler med.',
    'Two wooden rings with a small dangling crocheted ball inside each, attached partway along '
    'the chain on each side of the Felix medallion. These are what the child reaches for and '
    'rattles.')
add('rangler_txt',
    "Kulene: hekle to kuler, samme oppskrift som på Felix' smokkelenke: 6 fm i magisk ring, "
    'økn x 6 (12), 12 fm i 2 omganger, mink x 6 (6), fyll lett, fest av med god tråd igjen.',
    "The balls: crochet two balls, the same pattern as on Felix's pacifier clip: 6 sc in a "
...
add('rangler_ferdig',
    ...
    'selve ringen godt fast tett inntil hovedkjeden på hver side av Felix, med flere runder '
    ...
    'looser. Sew the ring itself securely close against the main chain on each side of Felix, '
...
add('kuler_lead',
    ...
    'rustoransje, kremhvitt og salviegrønt, med en liten tre-mellomperle mellom hver kule, som '
    ...
    'alternating rust orange, cream and sage green, with a small wooden spacer bead between '
...
add('montering_lead',
    'Legg Felix-medaljongen, blomsten, bladet, de to trering-ranglene og alle kulene ut i den '
    'rekkefølgen du vil ha dem, før du fester noe permanent.',
    'Lay out the Felix medallion, the flower, the leaf, the two wooden ring rattles and all the '
...
add('montering_steg', [
    ...
    'Sy Felix-medaljongen fast midt på kjeden, med volangkragen og minihalen ferdig heklet.',
...
add('montering_steg_en', [
    ...
    'Sew the Felix medallion onto the middle of the chain, with the ruffled collar and mini '
    'tail already crocheted on.',
    'Thread the balls onto the cord with a small wooden spacer bead between each one, evenly '
    'distributed on both sides of Felix.',
...
add('foto_captions',
    ['Felix-medaljongen med krage', 'Kjeden lagt ut', 'Trering-ranglene', 'Ferdig montert '
     'lenke'],
    ['The Felix medallion with collar', 'The chain laid out', 'The wooden ring rattles',
...
add('ferdig_txt',
    'Nå har du heklet en liten Felix-vognlenke. Husk å sjekke lengden en siste gang før den '
    'festes til vognen!',
    "Now you have crocheted a little Felix stroller toy. Remember to check the length one "
...
add('kolleksjon_liste',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Ellie's rattle", "Pip's stroller toy", "Molly's stroller toy", "Luna's stroller toy",
...
<p class="small center" style="margin-top:-2mm;">{"Bildet viser Felix som stiluttrykk-referanse, ikke selve vognlenken." if lang == 'no' else 'Photo shows Felix as a style reference, not the stroller toy itself.'}</p>
