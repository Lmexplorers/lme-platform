# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Pips rangle' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Pips rangle, LME hekleoppskrift", "Pip's Rattle, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;PIPS RANGLE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;PIP'S RATTLE")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'PIPS RANGLE', "PIP'S RATTLE")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten pinnsvin-rangle på en trering, ca. 13 til 15 cm høy. Hodet er formet som en '
    'miniatyrutgave av Pip, med myke ører, en liten piggstripe langs midten, en '
    'salviegrønn volangkrage rundt halsen og en rangleboks trygt gjemt inni.',
    'A little hedgehog rattle on a wooden ring, approx. 13 to 15 cm tall. The head is shaped '
    "like a miniature version of Pip, with soft ears, a little spike stripe down the centre, "
    "a sage green ruffled collar around the neck and a rattle capsule safely tucked inside.")
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM PIPS RANGLE", "ABOUT PIP'S RATTLE")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Pips rangle hører til LME Baby Collection \"Woodland Dreams\". Dette lille pinnsvinhodet "
    'på en trering er tenkt som barnets aller første møte med Pip, laget for hender som ennå '
    'er for små til å holde selve dyret.',
    "Pip's rattle belongs to the LME Baby Collection \"Woodland Dreams\". This little hedgehog "
    "head on a wooden ring is meant as the child's very first meeting with Pip, made for hands "
    "that are still too small to hold the toy itself.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Skandinavisk og Montessori-inspirert, i de samme naturfargene som resten av kolleksjonen: '
    'kremhvitt, mørkt varmt brunt og salviegrønt. Rund, myk form uten skarpe kanter.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection: cream, dark warm brown and sage green. A round, soft shape with no sharp '
    'edges.')
add('pill_funksjon', 'FUNKSJON', 'FUNCTION')
add('om_funksjon',
    'Trering gir en trygg, lett gripbar håndtak-form. Rangleboksen inni hodet lager en myk '
    'raslelyd. Piggstripen gir litt tekstur å utforske med fingrene.',
    'The wooden ring gives a safe, easy-to-grip handle shape. The rattle capsule inside the '
    'head makes a soft rustling sound. The spike stripe gives a little texture to explore with '
    'small fingers.')
...
add('garn_txt',
    "Bystrikk Merino, kremhvit (hovedfarge), mørkt varmt brunt (ører og piggstripe) og "
    'salviegrønt (kragen), samme garn som resten av Pips oppskrift.',
    "Bystrikk Merino, cream (main colour), dark warm brown (ears and spike stripe) and sage "
    "green (the collar), the same yarn as the rest of Pip's pattern.")
add('garn_alt',
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Pip selv, siden hodet er lite'),
...
add('ord_lead',
    'Hele rangelen hekles med fastmasker i spiral, unntatt piggstripen, som bruker samme '
    'løkketeknikk som Pips egne pigger. Her er forkortelsene, med de vanlige amerikanske '
    'hekletermene ved siden av.',
    "The whole rattle is crocheted with single crochet in a spiral, except the spike stripe, "
    "which uses the same loop technique as Pip's own spikes. Here are the abbreviations, with "
...
add('oversikt_deler', [
    ...
    ('2. Halsen og kragen', 'tube med salviegrønn volangkant, sys rundt treringen',
     '2. The neck and collar', 'a tube with a sage green ruffled edge, sewn around the '
     'wooden ring'),
    ('3. Ørene', 'to stk, myke, to lag', '3. The ears', 'two, soft, two layers'),
    ('4. Piggstripen', 'liten løkkemasket stripe langs midten', '4. The spike stripe',
     'a little loop-stitch stripe down the centre'),
])

# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: HODET', 'PART 1: THE HEAD')
add('hode_lead',
    'Hodet hekles i spiral, ovenfra og ned, i kremhvitt. Rangleboksen legges inn før de '
    'siste omgangene lukkes.',
    'The head is crocheted in a spiral, from the top down, in cream. The rattle capsule is '
...
add('krage_txt',
    'Før du syr hodet fast: fest salviegrønn tråd i halsens øverste kant (12 m). *1 fm i '
    'neste maske, hopp over 1 maske, 3 stav i neste maske (en liten vifte)*, gjenta rundt '
    'hele kanten (6 vifter totalt). Fest av og gjem tråden.',
    'Before sewing the head on: attach sage green yarn at the top edge of the neck (12 sts). '
...
# ---------------------------------------------------------------- SIDE 9: ØRER OG PIGGSTRIPE
add('banner_orer', 'DEL 3: ØRENE OG PIGGSTRIPEN', 'PART 3: THE EARS AND THE SPIKE STRIPE')
add('pill_orer', 'ØRENE (2 STK, TO LAG)', 'THE EARS (MAKE 2, TWO LAYERS)')
add('orer_txt',
    'Ytterside (mørkt varmt brunt): 6 fm i magisk ring, økn x 6 (12), (1 fm, økn) x 6 (18). '
    'Avslutt. Innside (kremhvitt): 6 fm i magisk ring, økn x 6 (12). Avslutt. Sy den lyse '
    'sirkelen midt oppå den mørke, og sy ørene fast øverst på hodet, ett på hver side.',
    'Outside (dark warm brown): 6 sc in a magic ring, inc x 6 (12), (1 sc, inc) x 6 (18). '
    'Fasten off. Inside (cream): 6 sc in a magic ring, inc x 6 (12). Fasten off. Sew the '
    'light circle onto the middle of the dark one, and sew the ears onto the top of the '
    'head, one on each side.')
add('pill_horn', 'PIGGSTRIPEN', 'THE SPIKE STRIPE')
add('horn_txt',
    'I stedet for en sløyfe får Pips rangle en miniatyr av signaturdelen hans: legg opp 4 lm '
    '+ 1 vendemaske i mørkt varmt brunt, 3 rader løkkm (4 løkkm hver rad). Klipp av med god '
    'tråd igjen. Sy piggstripen fast langs midtlinjen fra pannen og bakover, mellom ørene og '
    'ned bakhodet.',
    "Instead of a bow, Pip's rattle gets a miniature of his signature part: chain 4 + 1 "
    "turning chain in dark warm brown, 3 rows of loop st (4 loop st each row). Cut, leaving "
    "a long tail. Sew the spike stripe on along the centre line from the forehead and "
    "backward, between the ears and down the back of the head.")
...
add('montering_steg', [
    ...
    'Sy ørene fast øverst på hodet, og piggstripen langs midtlinjen.',
    'Brodér ansiktet.',
...
add('montering_steg_en', [
    ...
    'Sew the ears onto the top of the head, and the spike stripe along the centre line.',
    'Embroider the face.',
...
add('regler',
    ['Brukes alltid under tilsyn av en voksen, spesielt de første gangene, til du er trygg '
     ...
     'Løkkemaskene i piggstripen er myke og ufarlige, men bør sjekkes jevnlig for å se at '
    ...
    ['Always use under adult supervision, especially the first few times, until you are '
     ...
     'The loop stitches in the spike stripe are soft and harmless, but should be checked '
...
add('ferdig_txt',
    'Nå har du heklet en liten Pip-rangle. Rist den forsiktig og hør den myke lyden!',
    'Now you have crocheted a little Pip rattle. Give it a gentle shake and listen to the '
    'soft sound!')
