# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift (hvit RO-bøttehatt) som HTML klar for PDF-print."""
import base64, html, pathlib

BASE = pathlib.Path(__file__).parent
PHOTO = BASE / 'ro_ref.jpg'
...
# ---------- byggeklosser ----------
def page(body, num, right_label='LME HEKLING'):
  ...
  <div class="phead">
    ...
    <div class="ph2">LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;RO-BØTTEHATT</div>
  </div>
...
def grid_cells(rows, x0, y0, cell, color=BLUE):
    """Tegner et rutemønster (BIG_R/BIG_O/FLAG) direkte som SVG-firkanter."""
    out = []
    for r, row in enumerate(rows):
        for c, v in enumerate(row):
            if v == '#':
                out.append(f'<rect x="{x0+c*cell:.1f}" y="{y0+r*cell:.1f}" '
...
def cover_illustration():
    ...
    # dome (hvit bomull)
    p.append(f'<path d="M80,215 L80,150 Q80,48 230,48 Q380,48 380,150 L380,215 Z" fill="{CREAM}" stroke="#c9bfa6" stroke-width="2.5"/>')
    p.append(f'<path d="M230,48 Q188,112 165,210 M230,48 Q230,125 230,210 M230,48 Q272,112 295,210 M230,48 Q150,108 100,196 M230,48 Q310,108 360,196" stroke="#ded4bb" stroke-width="1.5" fill="none" opacity="0.7"/>')
    # lite flagg øverst på kappen
    fcell = 3.4
    fw = len(FLAG[0])*fcell
    fx = W/2 - fw/2
    for r, row in enumerate(FLAG):
        for c, v in enumerate(row):
            p.append(f'<rect x="{fx+c*fcell:.1f}" y="{72+r*fcell:.1f}" width="{fcell:.1f}" '
                     f'height="{fcell:.1f}" fill="{CMAP[v]}"/>')
    # RO i blått på kappen (BIG_R + BIG_O)
    lcell = 8.0
    lgap = lcell*1.6
    rw = len(BIG_R[0])*lcell
    ow = len(BIG_O[0])*lcell
    totw = rw + lgap + ow
    lx = W/2 - totw/2
    p.append(grid_cells(BIG_R, lx, 132, lcell))
    p.append(grid_cells(BIG_O, lx + rw + lgap, 132, lcell))
    # brem med bølgekant (helblå)
    ...
    while xw < 416:
        ...
        xw = nxt; up = not up
    wave += ' L380,215 Z'
    p.append(f'<path d="{wave}" fill="{BLUE}" stroke="#123a7a" stroke-width="2.5"/>')
    p.append('</svg>')
    return ''.join(p)
...
{card('<p class="center">En hvit bøttehatt heklet i bomull, med "RO" og det norske flagget i blått '
      'foran, og to blå bølgeskvulp bak. Bremmen er heklet i solid blått med bølget kant. Motivene '
      'hekles rett inn med tapestry-heikling, ikke brodert på. Passer voksen (hodeomkrets 54 til '
      '57 cm). En rolig og fin sommerhatt.</p>')}
<div class="byline">
...
{card('<p><b>DROPS Paris</b> (100 % bomull):</p>'
      '<table class="t"><tr><th>Farge</th><th>Til</th><th>Mengde</th></tr>'
      '<tr><td><span class="dot" style="background:#fff;border:1px solid #bbb"></span> Hvit (01)</td><td>hovedfarge</td><td>3 nøster</td></tr>'
...
# ============ SIDE 5: DEL 2 SIDENE ============
pages.append(page(f'''
{banner('DEL 2: SIDENE MED RO OG BØLGER')}
<p>Nå hekler du rett opp i spiral, uten økninger. Du har 78 masker på hver omgang. Hovedfargen
er hvit. Foran hekler du RO og flagget i blått (flagget med rødt og marineblått), og bak hekler
du to blå bølgeskvulp.</p>
{card(otab([
  ('14 til 17', '78 fm med hvit', 78),
  ('18 til 27', 'Hekle inn RO og flagget foran, og bølgene bak (se side 6)', 78),
  ('28 til 31', '78 fm med hvit', 78),
]))}
{pink('VIKTIG: TAPESTRY-HEIKLING')}
{card('<p>Motivene hekles rett inn i sidene med tapestry-heikling (innheklede masker), ikke '
...
# ============ SIDE 6: DIAGRAM RO OG FLAGGET (FORAN) ============
pages.append(page(f'''
{banner('DIAGRAM: RO OG FLAGGET (FORAN)')}
<p>Hver rute = 1 maske x 1 omgang. Blå rute: hekle med blått. Hvit rute: hekle med hvitt
(hovedfargen). Foran hekler du RO nederst, med det norske flagget litt over. Les nedenfra og opp.</p>
{tealp('RO (HVER BOKSTAV 7 RUTER BRED, 9 RUTER HØY)')}
<div class="chartrow tight">
{chart_svg(BIG_R, cell=20, title='R')}{chart_svg(BIG_O, cell=20, title='O')}
</div>
{pink('FLAGGET (13 RUTER BREDT, 10 RUTER HØYT)')}
<div class="chartrow tight">
...
# ============ SIDE 7: DIAGRAM BØLGENE (BAK) ============
pages.append(page(f'''
{banner('DIAGRAM: BØLGENE (BAK)')}
<p>Bak på hatten hekler du to blå bølgeskvulp, ett på hver side av midt bak. Les nedenfra og opp.</p>
<div class="chartrow tight">
{chart_svg(WAVE, cell=17, title='BØLGE VENSTRE (11 x 8)')}{chart_svg(WAVE_M, cell=17, title='BØLGE HØYRE (speilvendt)')}
</div>
{pink('HVOR PÅ HATTEN?')}
{card(ul([
...
# ============ SIDE 8: DEL 3 BREM ============
pages.append(page(f'''
{banner('DEL 3: BREMMEN (BLÅ, MED ØKNINGER)')}
<p>Nå bytter du til blått og hekler hele bremmen i blått. Bremmen bøyer seg utover, så du øker
jevnt annenhver omgang.</p>
{card(otab([
  (32, '78 fm med blått', 78),
  (33, 'økn x 12 jevnt fordelt', 90),
  (34, '90 fm med blått', 90),
  (35, 'økn x 12 jevnt fordelt', 102),
  (36, '102 fm med blått', 102),
  (37, 'økn x 12 jevnt fordelt', 114),
  (38, '114 fm med blått', 114),
]))}
...
# ============ SIDE 9: DEL 4 BØLGET KANT ============
pages.append(page(f'''
{banner('DEL 4: BØLGET KANT')}
<p>Helt ytterst lager du den bølgete kanten, fortsatt i blått.</p>
{card(otab([
  (39, 'økn x 18 jevnt fordelt', 132),
  (40, '132 fm med blått', 132),
  (41, 'Avslutt med kjedemasker (eller krepsemasker, valgfritt)', 132),
]))}
{cme('Vil du ha ekstra tydelige bølger? Avslutt slik rundt: 3 fm i samme maske, 1 fm, hopp over '
     '2 masker, og gjenta. Da bukter kanten seg fint, som skvulp i vannkanten.')}
''', 9))

# ============ SIDE 10: FERDIG OG STELL ============
pages.append(page(f'''
{banner('FERDIG OG STELL')}
{tealp('SLIK BLIR DET UNDERVEIS')}
{card(ul([
  '<b>1. Toppen:</b> en flat, rund hvit skive (ca. 16 cm).',
  '<b>2. Sidene:</b> rett opp i hvitt, med RO og flagget foran og bølgene bak.',
  '<b>3. Bremmen:</b> blå, og begynner å bøye seg utover.',
  '<b>4. Bølget kant:</b> ytterst i blått, som små skvulp.',
]))}
{pink('VASK OG PLEIE')}
{card(ul([
...
{cme('Gratulerer! Nå har du heklet din egen RO-hatt.')}
...
<title>RO-bøttehatt, LME hekleoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages)}</body></html>'''

(BASE / 'hekle_ro.html').write_text(doc, encoding='utf-8')
