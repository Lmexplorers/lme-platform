# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Felix' rangle' (norsk + engelsk) som HTML,
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Felix' rangle, LME hekleoppskrift", "Felix's Rattle, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;FELIX\' RANGLE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;FELIX'S RATTLE")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', "FELIX' RANGLE", "FELIX'S RATTLE")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten reve-rangle på en trering, ca. 13 til 15 cm høy. Hodet er formet som en '
    'miniatyrutgave av Felix, med spisse ører, en salviegrønn volangkrage rundt halsen, en '
    'liten tofarget "minihale" og en rangleboks trygt gjemt inni.',
    'A little fox rattle on a wooden ring, approx. 13 to 15 cm tall. The head is shaped like '
    'a miniature version of Felix, with pointed ears, a sage green ruffled collar around the '
    'neck, a little two-tone "mini tail", and a rattle capsule safely tucked inside.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM FELIX' RANGLE", "ABOUT FELIX'S RATTLE")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Felix' rangle hører til LME Baby Collection \"Woodland Dreams\". Dette lille revehodet "
    'på en trering er tenkt som barnets aller første møte med Felix, laget for hender som '
    'ennå er for små til å holde selve dyret.',
    "Felix's rattle belongs to the LME Baby Collection \"Woodland Dreams\". This little fox "
    "head on a wooden ring is meant as the child's very first meeting with Felix, made for "
...
add('om_stil',
    ...
    'rustoransje, kremhvitt og salviegrønt. Rund, myk form uten skarpe kanter.',
    'Scandinavian and Montessori-inspired, in the same natural colours as the rest of the '
    'collection: rust orange, cream and sage green. A round, soft shape with no sharp edges.')
add('pill_funksjon', 'FUNKSJON', 'FUNCTION')
add('om_funksjon',
    'Trering gir en trygg, lett gripbar håndtak-form. Rangleboksen inni hodet lager en myk '
    'raslelyd. Minihalen gir litt tekstur å utforske med fingrene.',
    'The wooden ring gives a safe, easy-to-grip handle shape. The rattle capsule inside the '
    'head makes a soft rustling sound. The mini tail gives a little texture to explore with '
    'small fingers.')
...
add('garn_txt',
    "Bystrikk Merino, rustoransje (hovedfarge), kremhvit (ører og minihale) og salviegrønt "
    "(kragen), samme garn som resten av Felix' oppskrift.",
    "Bystrikk Merino, rust orange (main colour), cream (ears and mini tail) and sage green "
    "(the collar), the same yarn as the rest of Felix's pattern.")
add('garn_alt',
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Felix selv, siden hodet er lite'),
...
add('ord_lead',
    ...
    'The whole rattle is crocheted with single crochet in a spiral. Here are the '
...
add('oversikt_deler', [
    ...
    ('2. Halsen og kragen', 'tube med salviegrønn volangkant, sys rundt treringen',
     ...
     'wooden ring'),
    ('3. Ørene', 'to stk, spisse, to lag', '3. The ears', 'two, pointed, two layers'),
    ('4. Minihalen', 'liten tofarget kule', '4. The mini tail', 'a little two-colour ball'),
])

# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: HODET', 'PART 1: THE HEAD')
add('hode_lead',
    'Hodet hekles i spiral, ovenfra og ned, i rustoransje. Rangleboksen legges inn før de '
    'siste omgangene lukkes.',
    'The head is crocheted in a spiral, from the top down, in rust orange. The rattle '
...
# ---------------------------------------------------------------- SIDE 9: ØRER OG MINIHALE
add('banner_orer', 'DEL 3: ØRENE OG MINIHALEN', 'PART 3: THE EARS AND THE MINI TAIL')
add('pill_orer', 'ØRENE (2 STK, TO LAG, SPISSE)', 'THE EARS (MAKE 2, TWO LAYERS, POINTED)')
add('orer_txt',
    'Ytterside (rustoransje): 6 fm i magisk ring, økn x 6 (12), (1 fm, økn) x 6 (18). '
    'Avslutt. Innside (kremhvitt): 6 fm i magisk ring, økn x 6 (12). Avslutt. Sy den lyse '
    'sirkelen midt oppå den oransje, fold hvert øre lett i to for en liten spiss, og sy '
    'ørene fast øverst på hodet, ett på hver side.',
    'Outside (rust orange): 6 sc in a magic ring, inc x 6 (12), (1 sc, inc) x 6 (18). Fasten '
    'off. Inside (cream): 6 sc in a magic ring, inc x 6 (12). Fasten off. Sew the light '
    'circle onto the middle of the orange one, fold each ear gently in half for a small '
    'point, and sew the ears onto the top of the head, one on each side.')
add('pill_horn', 'MINIHALEN', 'THE MINI TAIL')
add('horn_txt',
    'I stedet for en sløyfe får Felix sin rangle en miniatyr av signaturdelen hans: 6 fm i '
    'magisk ring, rustoransje (6). Økn x 6 (12). 2 omganger, 12 fm. Bytt til kremhvit, 1 '
    'omgang 12 fm. Mink x 6 (6). Fyll lett og klipp av med god tråd igjen. Sy minihalen fast '
    'til bakhodet, rett under ørene.',
    "Instead of a bow, Felix's rattle gets a miniature of his signature part: 6 sc in a "
    "magic ring, rust orange (6). Inc x 6 (12). 2 rounds, 12 sc. Switch to cream, 1 round 12 "
    "sc. Dec x 6 (6). Stuff lightly and cut, leaving a long tail. Sew the mini tail onto the "
    "back of the head, just below the ears.")
...
add('montering_steg', [
    ...
    'Sy ørene fast øverst på hodet, og minihalen på bakhodet.',
    'Brodér ansiktet.',
...
add('montering_steg_en', [
    ...
    'Sew the ears onto the top of the head, and the mini tail onto the back of the head.',
    'Embroider the face.',
...
add('ferdig_txt',
    'Nå har du heklet en liten Felix-rangle. Rist den forsiktig og hør den myke lyden!',
    'Now you have crocheted a little Felix rattle. Give it a gentle shake and listen to the '
    'soft sound!')
...
add('kolleksjon_liste',
    ['Ellie, det lille dådyret', 'Pip, det lille pinnsvinet', 'Felix, den lille reven',
     ...
     'Lunas smokkelenke', 'Olivers smokkelenke', 'Pips rangle', 'Ellies rangle',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Pip's rattle", "Ellie's rattle", "Ellie's stroller toy", "Ellie's ballerina shoes",
...
<p class="small center" style="margin-top:-2mm;">{'Bildet viser Felix som stiluttrykk-referanse, ikke selve rangelen.' if lang == 'no' else 'Photo shows Felix as a style reference, not the rattle itself.'}</p>
