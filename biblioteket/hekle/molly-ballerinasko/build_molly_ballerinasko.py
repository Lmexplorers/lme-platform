# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Mollys ballerinasko' (norsk + engelsk) som HTML,
...
REF = BASE / 'molly_ballerinasko_real.jpg'
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', 'Mollys ballerinasko, LME hekleoppskrift', "Molly's Ballerina Shoes, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;MOLLYS BALLERINASKO',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;MOLLY'S BALLERINA SHOES")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'MOLLYS BALLERINASKO', "MOLLY'S BALLERINA SHOES")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'Søte, klassiske babyballerinaer med rund tå, et lite lammeansikt med en tett løkkemasket '
    'ulltopp og en smørgul sløyfe øverst, myke ører i ett lag, T-stropp med knapp, en fin '
    'gulkantet picotkant og et hjerte brodert på sålen. Fem størrelser, fra prematur til '
    '12 måneder, pluss en matchende hårsløyfe. Heklet i de samme naturfargene som resten av '
    'Molly-kolleksjonen.',
    'Sweet, classic baby ballerina shoes with a round toe, a little lamb face with a dense '
    'loop-stitch wool topknot and a butter yellow bow on top, soft single-layer ears, a T-bar '
    'strap with a button, a pretty yellow-edged picot trim and a heart embroidered on the sole. '
    'Five sizes, from preemie to 12 months, plus a matching hair bow. Crocheted in the same '
    'natural colours as the rest of the Molly collection.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', 'OM MOLLYS BALLERINASKO', "ABOUT MOLLY'S BALLERINA SHOES")
add('pill_historien', 'DEL AV MOLLYS VERDEN', "PART OF MOLLY'S WORLD")
add('om_historien',
    'Mollys ballerinasko hører til LME Baby Collection "Woodland Dreams". Tenk deg at Molly '
    'selv skulle hatt et par små sko å tulle rundt i blomsterengen med, akkurat sånn er disse '
    'ballerinaene tenkt: myke, søte og klare for lek.',
    'Molly\'s ballerina shoes belong to the LME Baby Collection "Woodland Dreams". Imagine Molly '
    'herself with a little pair of shoes to toddle around the flower meadow in, that is exactly '
...
add('om_stil',
    ...
    'kolleksjonen: krem og lys brun, med Mollys signaturfarge smørgult på sløyfen og kantene. '
    'Kan mikses og matches, for eksempel kremfarget sko med gul sløyfe.',
    'A classic ballerina shoe shape with a round toe and T-bar strap, in the same natural '
    'colours as the rest of the collection: cream and light brown, with Molly\'s signature '
    'butter yellow on the bow and edging. Can be mixed and matched, for example cream shoes '
    'with a yellow bow.')
...
add('garn_txt',
    ...
    'liten rest av smørgult til sløyfen og picotkanten, og kremhvitt til lammets ulltopp (den '
    'fargen du ikke bruker på selve skoen). Samme garnfamilie som resten av Molly-kolleksjonen.',
    'Bystrikk Merino or DROPS Cotton Merino in cream or light brown for the shoe itself, plus a '
    'small leftover of butter yellow for the bow and picot trim, and cream for the lamb\'s wool '
    'topknot (whichever colour you are not using on the shoe itself). The same yarn family as '
    'the rest of the Molly collection.')
add('garn_alt',
...
add('ord_lead',
    'Skoene bruker fastmasker, løkkemasker til ulltoppen, og en pigg-teknikk (picot) til '
    'kanten. Her er forkortelsene, med de vanlige amerikanske hekletermene ved siden av.',
    'The shoes use single crochet, loop stitches for the wool topknot, and a picot technique '
    'for the edge. Here are the abbreviations, with the common US crochet terms alongside.')
...
add('ord_rows', [
    ...
    ('løkkm', 'loop st', 'løkkemaske: trekk en løkke av garnet gjennom maska og la den stå, '
     'brukes til lammets ulltopp'),
...
add('oversikt_deler', [
    ...
    ('3. Lammeansiktet', 'lite hode med ører og ulltopp på tåpartiet', '3. The lamb face', 'a little head with ears and a wool topknot on the toe'),
    ('4. T-stroppen', 'med lukking', '4. The T-bar strap', 'with a closure'),
    ('5. Picotkanten', 'dekorativ, gulkantet kant øverst', '5. The picot edge', 'a decorative, yellow-edged trim at the top'),
    ('6. Sløyfen', 'smørgul, oppå ulltoppen på hver sko', '6. The bow', 'butter yellow, on top of the wool topknot on each shoe'),
])
...
# ---------------------------------------------------------------- SIDE 10: LAMMEANSIKTET
add('banner_lam', 'DEL 3: LAMMEANSIKTET', 'PART 3: THE LAMB FACE')
add('lam_lead',
    'Et lite, flatt lammehode sys fast på tåpartiet av hver sko, i kontrastfargen til selve '
    'skoen, med en tett løkkemasket ulltopp øverst.',
    'A little, flat lamb head is sewn onto the toe of each shoe, in a colour that contrasts '
    'with the shoe itself, with a dense loop-stitch wool topknot on top.')
add('lam_rows', [
...
add('lam_ferdig',
    'Fest av med god tråd igjen, og press hodet flatt. Hekle to bittesmå ører i ett lag (5 fm i '
    'magisk ring, fest av, hekle 2 stk per sko) og sy dem fast på hver sin side av hodet, myke '
    'og litt slappe, akkurat som i referansebildet. Hekle deretter en tett ulltopp øverst på '
    'hodet: fest kremhvit tråd og lag 6 til 8 løkkemasker rett inn på overflaten mellom ørene. '
    'Brodér et lite ansikt (to øyne, en liten nese og gjerne rosa kinn) med samme teknikk som '
    'Mollys andre motiver. Sy lammeansiktet godt fast midt på tåpartiet av skoen.',
    'Fasten off, leaving a long tail, and press the head flat. Crochet two tiny single-layer '
    'ears (5 sc in a magic ring, fasten off, make 2 per shoe) and sew them onto either side of '
    'the head, soft and a little floppy, just like in the reference photo. Then crochet a '
    'dense wool topknot on top of the head: attach cream yarn and work 6 to 8 loop stitches '
    'directly onto the surface between the ears. Embroider a small face (two eyes, a little '
    'nose, and pink cheeks if you like) using the same technique as Molly\'s other motifs. Sew '
    'the lamb face securely onto the middle of the toe of the shoe.')
...
add('lukking_alt',
    [('Uten knapp (anbefalt for de minste)', 'La stroppen ende i en liten heklet løkke, og sy '
     ...
     ('Med myk heklet knapp', 'Hekle en liten flat "knapp" (5 fm i magisk ring, avslutt) og sy '
      'den godt og fast til siden av skoen, som i referansebildet. Tre stroppens løkke over '
      'knappen. Bruk aldri en hard plast- eller metallknapp, og sjekk at knappen sitter godt '
      'fast før hver bruk.')],
    [('Without a button (recommended for the smallest sizes)', 'Let the strap end in a small '
     ...
     ('With a soft crocheted button', 'Crochet a small flat "button" (5 sc in a magic ring, '
      'fasten off) and sew it securely onto the side of the shoe, as in the reference photo. '
      'Slip the strap\'s loop over the button. Never use a hard plastic or metal button, and '
      'check that the button is firmly attached before every use.')])
...
add('picot_lead',
    ...
    'helt til slutt rundt hele den øvre åpningen, i smørgult for et fint fargeglimt mot '
    'kremfargen.',
    'The picot edge gives the shoe its classic, slightly lace-like ballerina look, and is '
    'crocheted last, all the way around the top opening, in butter yellow for a lovely pop of '
    'colour against the cream.')
add('picot_txt',
...
add('sloyfe_lead',
    'En liten smørgul sløyfe sys fast oppå ulltoppen på hver sko.',
    "A small butter yellow bow is sewn on top of the wool topknot on each shoe.")
add('sloyfe_rows', [
...
add('sloyfe_ferdig',
    ...
    'garn stramt rundt midjen, slik at det blir en liten sløyfeform. Sy sløyfen fast oppå '
    'ulltoppen, akkurat som i referansebildet.',
    ...
    'top of the wool topknot, just like in the reference photo.')
...
add('montering_steg', [
    ...
    'Hekle lammeansiktene med ører og ulltopp og brodér ansiktene, og sy ett fast på tåpartiet '
    'av hver sko.',
    'Hekle T-stroppen på hver sko, og fest med valgt lukking (uten knapp, eller med myk heklet '
    'knapp).',
    'Hekle den smørgule picotkanten rundt overkanten på begge skoene.',
    'Hekle to sløyfer og sy dem fast oppå ulltoppen på hvert lammeansikt.',
...
add('montering_steg_en', [
    ...
    'heart on each sole.',
    'Crochet the lamb faces with ears and wool topknot and embroider the faces, and sew one '
    'onto the toe of each shoe.',
    'Crochet the T-bar strap on each shoe, and fasten with your chosen closure (no button, or '
    'a soft crocheted button).',
    'Crochet the butter yellow picot edge around the top of both shoes.',
    'Crochet two bows and sew them on top of the wool topknot on each lamb face.',
...
add('sikkerhet_txt',
    ['Ingen deler limes. Alt sys fast med sterk, tvinnet bomullstråd og mange, tette sting.',
     ...
     'Sjekk skoene jevnlig for slitasje: løse tråder, lammeansikt, ører, ulltopp, sløyfer eller '
     'knapper som sitter løst. Kast eller reparer umiddelbart hvis noe er galt.',
    ...
    ['No parts are glued. Everything is sewn on with strong, twisted cotton thread and plenty '
     ...
     'Check the shoes regularly for wear: loose threads, a loose lamb face, ears, wool '
     'topknot, bows or buttons that feel loose. Discard or repair immediately if anything is '
     'wrong.',
...
add('ferdig_txt',
    'Nå har du heklet et lite par ballerinasko til Molly-kolleksjonen. Perfekt til bilder, '
    'gaver og hverdagskos!',
    "Now you have crocheted a little pair of ballerina shoes for the Molly collection. Perfect "
...
add('kolleksjon_liste',
    ...
    ['Molly, the little lamb', "Molly's pacifier clip", "Molly's rattle",
     "Molly's stroller toy", "Molly's activity toy"])
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="coverimg"><img src="{ref_src}" alt="Mollys ballerinasko, stiluttrykk-referanse"></div>
...
''', 9))
    ...
    lam_rows = T['lam_rows']['no'] if lang == 'no' else T['lam_rows_en']['no']
    pages.append(pg(f'''
{banner(t('banner_lam'))}
<p>{t('lam_lead')}</p>
{card(otab(lam_rows, head3[lang]))}
{cme(t('lam_ferdig'))}
''', 10))
