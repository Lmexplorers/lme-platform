# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Lunas ballerinasko' (norsk + engelsk) som HTML,
...
REF = BASE / 'luna_ballerinasko_real.jpg'
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', 'Lunas ballerinasko, LME hekleoppskrift', "Luna's Ballerina Shoes, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;LUNAS BALLERINASKO',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;LUNA'S BALLERINA SHOES")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'LUNAS BALLERINASKO', "LUNA'S BALLERINA SHOES")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'Søte, klassiske babyballerinaer med rund tå, et lite kaninansikt med lange, avlange ører '
    'og en rosa sløyfe mellom dem på tåpartiet, T-stropp med en myk rosa knapp, en fin rosa kant '
    'øverst og et rosa hjerte brodert på sålen. Fem størrelser, fra prematur til 12 måneder, '
    'pluss en matchende hårsløyfe. Heklet i Lunas egne farger: varmt grått og rosa, med '
    'kremhvite innslag i ørene.',
    'Sweet, classic baby ballerina shoes with a round toe, a little bunny face with long, '
    'elongated ears and a pink bow between them on the toe, a T-bar strap with a soft pink '
    'button, a pretty pink edge at the top and a pink heart embroidered on the sole. Five '
    'sizes, from preemie to 12 months, plus a matching hair bow. Crocheted in Luna\'s own '
    'colours: warm grey and pink, with cream accents in the ears.')
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', 'OM LUNAS BALLERINASKO', "ABOUT LUNA'S BALLERINA SHOES")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    'Lunas ballerinasko hører til LME Baby Collection "Woodland Dreams". Tenk deg at Luna selv '
    'skulle hatt et par små sko å hoppe rundt i blomsterengen med, akkurat sånn er disse '
    'ballerinaene tenkt: myke, søte og klare for lek, med de lange ørene hennes titter opp '
    'over kanten.',
    'Luna\'s ballerina shoes belong to the LME Baby Collection "Woodland Dreams". Imagine Luna '
    'herself with a little pair of shoes to hop around the flower meadow in, that is exactly '
    'the feeling these ballerinas are meant to have: soft, sweet and ready for play, with her '
    'long ears peeking up over the top edge.')
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Klassisk ballerinasko-form med rund tå og T-stropp, i Lunas egne farger: varmt grått for '
    'selve skoen og rosa for sløyfen, kanten og knappen, med kremhvite innslag i ørenes '
    'innerside og kaninansiktets snuteflekk.',
    'A classic ballerina shoe shape with a round toe and T-bar strap, in Luna\'s own colours: '
    'warm grey for the shoe itself and pink for the bow, the edge and the button, with cream '
    'accents on the inside of the ears and the bunny face\'s muzzle patch.')
...
add('garn_txt',
    'Bystrikk Merino eller DROPS Cotton Merino i varmt grått til selve skoen, pluss en liten '
    'rest av rosa til sløyfen, kanten og knappen, og kremhvitt til kaninansiktets snuteflekk og '
    'ørenes innerside. Samme garnfamilie som resten av Luna-kolleksjonen.',
    'Bystrikk Merino or DROPS Cotton Merino in warm grey for the shoe itself, plus a small '
    'leftover of pink for the bow, the edge and the button, and cream for the bunny face\'s '
    'muzzle patch and the inside of the ears. The same yarn family as the rest of the Luna '
    'collection.')
add('garn_alt',
...
add('oversikt_deler', [
    ...
    ('3. Kaninansiktet', 'lite hode med lange, avlange ører på tåpartiet', '3. The bunny face',
     'a little head with long, elongated ears on the toe'),
    ('4. T-stroppen', 'med lukking', '4. The T-bar strap', 'with a closure'),
    ('5. Kanten', 'dekorativ kant øverst', '5. The edge', 'a decorative trim at the top'),
...
add('sale_ferdig',
    ...
    'rosa hjerte midt på undersiden av sålen, som i referansebildet, et søtt detalj som vises '
    'når babyen løfter foten.',
    'Do not cut the yarn. You continue straight into the upper from the same thread. Feel free '
    'to embroider a small pink heart in the middle of the underside of the sole, as in the '
...
add('overdel_lead',
    'Overdelen hekles rett opp fra sålens ytterkant, i varmt grått.',
    'The upper is crocheted straight up from the outer edge of the sole, in warm grey.')
add('overdel_rows', [
...
# ---------------------------------------------------------------- SIDE 10: KANINANSIKTET
add('banner_kanin', 'DEL 3: KANINANSIKTET', 'PART 3: THE BUNNY FACE')
add('kanin_lead',
    'Et lite, flatt kaninhode sys fast på tåpartiet av hver sko, med lange, avlange ører, '
    'akkurat som Lunas signaturkjennetegn på resten av kolleksjonen.',
    "A little, flat bunny head is sewn onto the toe of each shoe, with long, elongated ears, "
    "just like Luna's signature feature on the rest of the collection.")
add('kanin_rows', [
...
add('pill_orer_kanin', 'ØRENE (2 STK, TO LAG)', 'THE EARS (MAKE 2, TWO LAYERS)')
add('orer_kanin_ute',
    'Yttersiden, varmt grått, hekle 2: legg opp 5 lm + 1 vendemaske, 5 fm tilbake (5). '
    'Rad 2: økn, 3 fm, økn (7). Rad 3 til 8: rett, 7 fm, 6 rader (7). Rad 9: mink, 3 fm, mink '
    '(5). Fest av med god tråd igjen.',
    'The outside, warm grey, make 2: chain 5 + 1 turning chain, 5 sc back across (5). Row 2: '
    'inc, 3 sc, inc (7). Rows 3 to 8: straight, 7 sc, 6 rows (7). Row 9: dec, 3 sc, dec (5). '
    'Fasten off, leaving a long tail.')
add('orer_kanin_inne',
    'Innsiden, kremhvitt, hekle 2: legg opp 4 lm + 1 vendemaske, 4 fm tilbake (4). '
    'Rad 2: økn, 2 fm, økn (6). Rad 3 til 6: rett, 6 fm, 4 rader (6). Rad 7: mink, 2 fm, mink '
    '(4). Fest av med god tråd igjen.',
    'The inside, cream, make 2: chain 4 + 1 turning chain, 4 sc back across (4). Row 2: inc, 2 '
    'sc, inc (6). Rows 3 to 6: straight, 6 sc, 4 rows (6). Row 7: dec, 2 sc, dec (4). Fasten '
    'off, leaving a long tail.')
add('kanin_ferdig',
    'Legg den kremhvite innsiden midt oppå den grå ytterdelen, litt forskjøvet mot den ene '
    'langsiden, og sy den fast med heftesting slik at en jevn grå kant vises rundt. Sy de to '
    'ferdige ørene fast øverst på hodet, tett inntil hverandre med plass til sløyfen mellom, og '
    'la den øverste tredjedelen av hvert øre stikke opp over skoens overkant, akkurat som i '
    'referansebildet. Brodér et lite ansikt (to øyne, en liten rosa nese, rosa kinn og gjerne '
    'to tynne brune øyenbryn, som gir Luna det søte uttrykket hennes) med samme teknikk som '
    'resten av Luna-kolleksjonen. Sy kaninansiktet godt fast midt på tåpartiet av skoen.',
    'Place the cream inside in the middle of the grey outer piece, slightly offset towards one '
    'long side, and sew it on with running stitch so an even grey rim shows around it. Sew the '
    'two finished ears onto the top of the head, close together with room for the bow between '
    'them, and let the top third of each ear peek up over the top edge of the shoe, just like '
    'in the reference photo. Embroider a small face (two eyes, a little pink nose, pink cheeks '
    'and, if you like, two thin brown eyebrows, which give Luna her sweet expression) using the '
    'same technique as the rest of the Luna collection. Sew the bunny face securely onto the '
    'middle of the toe of the shoe.')
...
add('lukking_alt',
    [('Uten knapp (anbefalt for de minste)', 'La stroppen ende i en liten heklet løkke, og sy '
      ...
      'minste størrelsene.'),
     ('Med myk heklet knapp', 'Hekle en liten flat "knapp" i rosa (5 fm i magisk ring, avslutt), '
      'som i referansebildet, og sy den godt og fast til siden av skoen. Tre stroppens løkke '
    ...
    [('Without a button (recommended for the smallest sizes)', 'Let the strap end in a small '
     ...
     ('With a soft crocheted button', 'Crochet a small flat "button" in pink (5 sc in a magic '
      'ring, fasten off), as in the reference photo, and sew it securely onto the side of the '
...
# ---------------------------------------------------------------- SIDE 12: KANTEN
add('banner_picot', 'DEL 5: KANTEN', 'PART 5: THE EDGE')
add('picot_lead',
    'Kanten gir skoen det klassiske, litt kniplingaktige ballerina-utseendet, og hekles helt '
    'til slutt rundt hele den øvre åpningen, i rosa.',
    'The edge gives the shoe its classic, slightly lace-like ballerina look, and is crocheted '
    'last, all the way around the top opening, in pink.')
add('picot_txt',
    'Fest rosa tråd hvor som helst langs overkanten. Hekle *1 fm i neste maske, 3 lm, kjm '
    ...
    'Attach pink yarn anywhere along the top edge. Crochet *1 sc in the next stitch, 3 ch, sl '
...
add('sloyfe_lead',
    'En liten rosa sløyfe sys fast mellom ørene, øverst på kaninansiktet på hver sko.',
    "A small pink bow is sewn on between the ears, at the top of the bunny face on each shoe.")
add('sloyfe_rows', [
...
add('sloyfe_ferdig',
    ...
    'between the ears, at the top of the bunny face.')
...
add('montering_steg', [
    'Hekle sålen og overdelen i ett, for begge skoene, og brodér gjerne et rosa hjerte på hver '
    'såle.',
    'Hekle kaninansiktene med lange, avlange ører (ytterside i varmt grått, innerside i '
    'kremhvitt) og brodér ansiktene, og sy ett fast på tåpartiet av hver sko.',
    'Hekle T-stroppen på hver sko, og fest med valgt lukking (uten knapp, eller med myk heklet '
    'knapp i rosa).',
    'Hekle den rosa kanten rundt overkanten på begge skoene.',
    'Hekle to rosa sløyfer og sy dem fast mellom ørene på hvert kaninansikt.',
...
add('montering_steg_en', [
    ...
    'pink heart on each sole.',
    'Crochet the bunny faces with long, elongated ears (outside in warm grey, inside in cream) '
    'and embroider the faces, and sew one onto the toe of each shoe.',
    'Crochet the T-bar strap on each shoe, and fasten with your chosen closure (no button, or '
    'a soft crocheted button in pink).',
    'Crochet the pink edge around the top of both shoes.',
    'Crochet two pink bows and sew them on between the ears on each bunny face.',
...
add('sikkerhet_txt',
    ['Ingen deler limes. Alt sys fast med sterk, tvinnet bomullstråd og mange, tette sting.',
     ...
     'Sjekk skoene jevnlig for slitasje: løse tråder, kaninansikt, ører, sløyfer eller knapper '
    ...
    ['No parts are glued. Everything is sewn on with strong, twisted cotton thread and plenty '
     ...
     'Check the shoes regularly for wear: loose threads, a loose bunny face, ears, bows or '
...
add('ferdig_txt',
    'Nå har du heklet et lite par ballerinasko til Luna-kolleksjonen. Perfekt til bilder, '
    'gaver og hverdagskos!',
    "Now you have crocheted a little pair of ballerina shoes for the Luna collection. Perfect "
...
add('kolleksjon_liste',
    ['Ellie, det lille dådyret', 'Pip, det lille pinnsvinet', 'Felix, den lille reven',
     ...
     'Lunas vognlenke', 'Ellies ballerinasko', 'Ellies aktivitetsleke', 'Lunas aktivitetsleke'],
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Ellie's ballerina shoes", "Ellie's activity toy", "Luna's activity toy"])
...
def build(lang):
    ...
    pages.append(pg(f'''
<div class="coverimg"><img src="{ref_src}" alt="Lunas ballerinasko, stiluttrykk-referanse"></div>
...
''', 9))
    ...
    kanin_rows = T['kanin_rows']['no'] if lang == 'no' else T['kanin_rows_en']['no']
    pages.append(pg(f'''
{banner(t('banner_kanin'))}
<p>{t('kanin_lead')}</p>
{card(otab(kanin_rows, head3[lang]))}
{sagep(t('pill_orer_kanin'))}
{card('<p>' + t('orer_kanin_ute') + '</p><p>' + t('orer_kanin_inne') + '</p>')}
{cme(t('kanin_ferdig'))}
''', 10))
