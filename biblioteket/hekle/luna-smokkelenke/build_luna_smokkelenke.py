# -*- coding: utf-8 -*-
"""Genererer LME-hekleoppskrift 'Lunas smokkelenke' (norsk + engelsk) som HTML,
...
FACE_REF = BASE / 'luna_face_ref.jpg'
...
# ---------------------------------------------------------------- SIDE 1
add('doctitle', "Lunas smokkelenke, LME hekleoppskrift", "Luna's Pacifier Clip, LME crochet pattern")
add('ph2', 'LME HEKLEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;LUNAS SMOKKELENKE',
    "LME CROCHET PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;LUNA'S PACIFIER CLIP")
add('covertag', 'LME HEKLEOPPSKRIFT - BABY', 'LME CROCHET PATTERN - BABY')
add('covertitle', 'LUNAS SMOKKELENKE', "LUNA'S PACIFIER CLIP")
add('subpill', 'LME BABY COLLECTION - WOODLAND DREAMS', 'LME BABY COLLECTION - WOODLAND DREAMS')
add('cover_desc',
    'En liten smokkelenke med et forenklet kaninhode inspirert av Luna, med en rosa '
    'volangkrage rundt halsen, to små, avlange ører som en miniatyr av hennes lange ører, en '
    'rosa sløyfe, og noen myke heklede kuler med en stjerneperle. Heklet i de samme '
    'naturfargene som resten av kolleksjonen. Maks lengde er satt for å følge gjeldende '
    'sikkerhetsanbefaling for smokkeholdere.',
    "A little pacifier clip with a simplified bunny head inspired by Luna, with a pink "
    "ruffled collar around the neck, two small, elongated ears as a miniature of her long "
    "ears, a pink bow, and a few soft crocheted balls with a star bead. Crocheted in the "
    "same natural colours as the rest of the collection. Maximum length follows current "
...
# ---------------------------------------------------------------- SIDE 2
add('banner_om', "OM LUNAS SMOKKELENKE", "ABOUT LUNA'S PACIFIER CLIP")
add('pill_historien', 'DEL AV ELLIES VERDEN', "PART OF ELLIE'S WORLD")
add('om_historien',
    "Lunas smokkelenke hører til LME Baby Collection \"Woodland Dreams\", samme skogsunivers "
    'som Luna, den lille kaninen. Det lille kaninhodet på lenken er en forenklet utgave av '
    'Luna, akkurat stor nok til å henge trygt og lekent ved siden av smokken.',
    "Luna's pacifier clip belongs to the LME Baby Collection \"Woodland Dreams\", the same "
    "woodland world as Luna, the little bunny. The small bunny head on the clip is a "
    "simplified version of Luna, just the right size to hang safely and playfully next to "
    "the pacifier.")
add('pill_stil', 'STIL', 'STYLE')
add('om_stil',
    'Minimalistisk, Montessori-inspirert og skandinavisk, i de samme naturfargene som Luna: '
    'varmt grått og rosa. Rolig og enkelt, aldri overlesset.',
    'Minimalist, Montessori-inspired and Scandinavian, in the same natural colours as Luna: '
    'warm grey and pink. Calm and simple, never cluttered.')
...
add('garn_txt',
    "Bystrikk Merino (varmt grått og rosa). Samme garn som resten av Lunas oppskrift, så "
    'restene ofte strekker til.',
    "Bystrikk Merino (warm grey and pink). Same yarn as the rest of Luna's pattern, so "
...
add('utstyr', [
    ('Heklenål 3 eller 3,5 mm', 'litt tettere enn Luna selv, siden delene er små'),
...
add('oversikt_deler', [
    ('1. Det lille kaninhodet', 'forenklet Luna med avlange mini-ører', '1. The little bunny '
     'head', 'a simplified Luna with elongated mini ears'),
    ('2. Volangkragen', 'rosa, rundt halsen', '2. The ruffled collar', 'pink, around the '
     'neck'),
    ('3. Sløyfen', 'rosa, mellom ørene', '3. The bow', 'pink, between the ears'),
...
# ---------------------------------------------------------------- SIDE 7: HODET
add('banner_hode', 'DEL 1: DET LILLE KANINHODET', 'PART 1: THE LITTLE BUNNY HEAD')
add('hode_lead',
    'En forenklet, liten utgave av Lunas hode, varmt grått, uten egen snuteflekk.',
    "A simplified, small version of Luna's head, warm grey, without a separate muzzle patch.")
...
add('pill_orer_mini', 'TO SMÅ, AVLANGE ØRER', 'TWO SMALL, ELONGATED EARS')
add('orer_mini_txt',
    'I stedet for runde ører får Lunas smokkelenke en miniatyr av signaturdelen hennes: legg '
    'opp 4 lm + 1 vendemaske, 3 rader fm (4 fm) i varmt grått, for hvert øre. Sy dem fast '
    'øverst på hodet, med litt avstand mellom, hengende lett nedover som en miniatyr av '
    'Lunas lange ører.',
    "Instead of round ears, Luna's pacifier clip gets a miniature of her signature part: "
    "chain 4 + 1 turning chain, 3 rows of sc (4 sc) in warm grey, for each ear. Sew them onto "
    "the top of the head, with a little space between, hanging gently downward as a "
    "miniature of Luna's long ears.")
add('pill_ansikt_mini', 'ANSIKTET, BRODERT', 'THE FACE, EMBROIDERED')
add('ansikt_mini_txt',
    'Brodér to små, runde øyne i svart satengsting og en liten trekantet nese under. Ikke '
    'bruk sikkerhetsøyne med plastdeler her, siden lenken skal være helt uten harde eller '
    'løse deler. Se referansebildet fra Luna for inspirasjon til uttrykket, i miniatyr.',
    ...
    "loose parts. See the reference photo from Luna for inspiration on the expression, in "
    "miniature.")

# ---------------------------------------------------------------- SIDE 8: VOLANGKRAGEN
add('banner_krage', 'DEL 2: VOLANGKRAGEN', 'PART 2: THE RUFFLED COLLAR')
add('pill_krage', 'VOLANGKRAGEN (ROSA)', 'THE RUFFLED COLLAR (PINK)')
add('krage_txt',
    'Fest rosa tråd rundt kanten der hodet er som smalest, nederst. *1 fm i neste maske, '
    'hopp over 1 maske, 3 stav i neste maske (en liten vifte), hopp over 1 maske*, gjenta '
    'rundt hele kanten. Fest av og gjem tråden.',
    'Attach pink yarn around the edge where the head is narrowest, at the bottom. *1 sc in '
    'the next stitch, skip 1 stitch, 3 dc in the next stitch (a little fan), skip 1 stitch*, '
    'repeat all the way around the edge. Fasten off and weave in the end.')
...
add('pill_sloyfe', 'SLØYFEN (ROSA)', 'THE BOW (PINK)')
add('sloyfe_txt',
    'Hekle en smal strimmel i rosa: legg opp 10 lm + 1 vendemaske, 1 rad fm (10 fm). Fold '
    'strimmelen sammen til en liten sløyfeform og sy et lite stykke garn stramt rundt midten '
    'flere ganger.',
    'Crochet a narrow strip in pink: chain 10 + 1 turning chain, 1 row of sc (10 sc). Fold '
    'the strip together into a small bow shape and wrap a short length of yarn tightly '
    'around the centre a few times.')
add('pill_sloyfe_fest', 'FEST SLØYFEN TIL HODET', 'ATTACH THE BOW TO THE HEAD')
add('sloyfe_fest_txt',
    'Sy sløyfen fast permanent og flatt mellom ørene, øverst på hodet, akkurat som i '
    'referansebildet.',
    'Sew the bow on permanently and flat, between the ears, on top of the head, just like in '
    'the reference photo.')

# ---------------------------------------------------------------- SIDE 10: KULENE
add('banner_kuler', 'DEL 4: DE HEKLEDE KULENE', 'PART 4: THE CROCHETED BALLS')
add('kuler_lead',
    'Hekle så mange kuler du trenger for å fylle ut lengden mellom klipsen og kaninhodet, '
    'gjerne i en rytme av varmt grått og rosa, med treperler mellom og én stjerneformet '
    'treperle et sted i rekken. Husk: total lengde skal aldri bli mer enn 22 cm.',
    'Crochet as many balls as you need to fill out the length between the clip and the bunny '
    'head, for example alternating warm grey and pink, with wooden beads in between and one '
    'star-shaped wooden bead somewhere in the row. Remember: the total length must never '
    'exceed 22 cm.')
add('kuler_rows', [
    ('1', '6 fm i magisk ring', 6),
    ('2', 'økn x 6', 12),
...
add('lokke_txt',
    ...
    'varmt grått: legg opp 16 lm + 1 vendemaske, 1 rad fm tilbake (16 fm). Fold strimmelen på '
    ...
    'strip in warm grey: chain 16 + 1 turning chain, 1 row of sc back (16 sc). Fold the strip '
...
add('montering_steg', [
    'Hekle volangkragen rundt nederste kant av kaninhodet, og sy sløyfen fast mellom ørene.',
    ...
    'Fest kaninhodet i den ene enden.',
...
add('montering_steg_en', [
    'Crochet the ruffled collar around the bottom edge of the bunny head, and sew the bow on '
    'between the ears.',
    ...
    'Attach the bunny head at one end.',
...
add('foto_captions',
    ['Kaninhodet ferdig', 'Sløyfen', 'Kulene på rekke', 'Ferdig montert lenke'],
    ['The finished bunny head', 'The bow', 'The balls in a row', 'The fully assembled clip'])
...
add('ferdig_txt',
    'Nå har du heklet din egen smokkelenke inspirert av Luna. Husk å sjekke lengden en siste '
    'gang før den tas i bruk!',
    "Now you have crocheted your own pacifier clip inspired by Luna. Remember to check the "
...
add('kolleksjon_liste',
    ...
    ['Ellie, the little fawn', 'Pip, the little hedgehog', 'Felix, the little fox',
     ...
     "Molly's pacifier clip", "Oliver's pacifier clip", "Ellie's rattle",
...
<div class="twocol">
  ...
  <div class="figwrap"><img src="{face_src}" alt="Luna ansikt, referanse"><div class="figcap small">{'Referanse: Lunas ansiktsuttrykk' if lang == 'no' else "Reference: Luna's expression"}</div></div>
</div>
''', 7))
