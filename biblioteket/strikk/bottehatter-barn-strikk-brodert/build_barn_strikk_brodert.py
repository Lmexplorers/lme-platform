# -*- coding: utf-8 -*-
"""Genererer LME-strikkeoppskrift (Bøttehatter barn, brodert/maskesting) som HTML klar for PDF-print."""
...
def dupstitch_panels(L):
    """3-panels maskesting-illustrasjon. L(no,en) -> tekst."""
...
pages.append(make_page('LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;BØTTEHATTER BARN, BRODERT')(
    ...
    'strikker først en stripet brem som bølger nedover, deretter hoveddelen rett opp, og til slutt felles '
    'toppen ned til en liten rundet topp. Helt til slutt broderer du motivet på med maskesting (kalles også '
    'duplikatsting), en søm som legger seg oppå strikken og ser strikket ut. Denne oppskriften dekker tre '
    'motiver, velg det du vil lage:</p>' +
    card(ul([
        '<b>NORGE</b>: ordet brodert i hvitt tvers over pannen',
...
pages.append(make_page('LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;BØTTEHATTER BARN, BRODERT')(
    ...
    card('<p>Et glatt bomullsgarn (aran/tykkelse 4) som gir 17 masker x 22 omganger glattstrikk = 10 x 10 cm '
         ...
         f'<tr><td><span class="dot" style="background:{WHITE};border-color:#ccc;"></span> Hvit</td><td>broderi: bokstaver, striper</td></tr>'
         f'<tr><td><span class="dot" style="background:{NAVY}"></span> Marineblå</td><td>striper, broderi: flagg, RO-brem</td></tr></table>'
         '<p class="small">Ha rikelig av rød hovedfarge (nesten hele hatten er strikket i rødt) og ett lite '
         'nøste hver av hvitt og marineblått, de brukes bare i bremmestripene og til broderiet.</p>') +
...
pages.append(make_page('LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;BØTTEHATTER BARN, BRODERT')(
    ...
    tealp('AVSLUTNING') +
    card('<p>Fest alle løse tråder godt på innsiden, både etter bremmestripene og etter broderiet. '
...
pages.append(page(
    ...
    'bottom up, in a single colour throughout. You start with a striped brim that flares, then the main '
    ...
    'of the knitting and looks knitted in. This pattern covers three motifs, pick the one you want to '
    'make:</p>' +
    card(ul([
        '<b>NORGE</b>: the word embroidered in white across the forehead',
...
pages.append(page(
    ...
    card('<p>A smooth cotton yarn (aran weight) that gives 17 stitches x 22 rounds in stockinette = 10 x 10 '
         ...
         f'<tr><td><span class="dot" style="background:{WHITE};border-color:#ccc;"></span> White</td><td>embroidery: letters, stripes</td></tr>'
         f'<tr><td><span class="dot" style="background:{NAVY}"></span> Navy</td><td>stripes, embroidery: flag, RO brim</td></tr></table>'
         '<p class="small">Have plenty of red main colour (almost the whole hat is knitted in red) and one '
         'small ball each of white and navy, they are only used in the brim stripes and the embroidery.</p>') +
...
# ============ PAGE 9: PART 3 HOW TO EMBROIDER ============
pages.append(page(
...
pages.append(page(
    ...
    tealp('FINISHING') +
    card('<p>Weave in all loose ends securely on the inside, both after the brim stripes and after the '
         'embroidery. Check that the embroidered stitches sit loose and relaxed on top of the knitting, '
         'not tight.</p>') +
    tealp('CARE') +
...
<title>Bøttehatter til baby og barn, brodert, LME strikkeoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages_no)}</body></html>'''

doc_en = f'''<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<title>Bucket hats for baby and child, duplicate stitch, LME knitting pattern</title>
<style>{css}</style></head>
<body>{''.join(pages_en)}</body></html>'''

(BASE / 'barn_strikk_brodert_no.html').write_text(doc_no, encoding='utf-8')
(BASE / 'barn_strikk_brodert_en.html').write_text(doc_en, encoding='utf-8')
