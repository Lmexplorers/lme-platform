# -*- coding: utf-8 -*-
"""RETTELSE: RO-bøttehatt barn, brodert, skal være samme design som RO-bøttehatt
for VOKSNE (hvit hoveddel, blå bølgende brem, RO+flagg foran, bølger bak, motivene
lagt til med maskesting/duplikatsting etterpå), ikke en rød bokstav-hatt som
NORGE/NORWAY. Denne overskriver kun RO-varianten fra
_build_barn_strikk_brodert_split.py, NORGE og NORWAY er allerede riktige og rørt ikke."""
import base64, html, pathlib

BASE = pathlib.Path(__file__).parent
OUT_DIR = BASE / 'bottehatter-barn-strikk-brodert-ro'
...
CMAP = {'.': WHITE, '#': BLUE, 'R': RED, 'W': '#ffffff', 'B': NAVY}
...
def chart_svg(rows, cell=18, numbers=False):
    ...
    for y, row in enumerate(rows):
        for x, ch in enumerate(row):
            fill = CMAP[ch]
            stroke = 'rgba(0,0,0,0.28)' if ch != '.' else 'rgba(0,0,0,0.15)'
            p.append(f'<rect x="{4+x*cell}" y="{4+y*cell}" width="{cell}" height="{cell}" '
                     f'fill="{fill}" stroke="{stroke}" stroke-width="1" '
                     f'stroke-dasharray="{"2 2" if ch == "." else "none"}"/>')
...
def vgrid(cols, rows, sw=34, sh=26, ox=10, oy=14, hi=None, done=None):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            out.append(f'<path d="{wpath}" fill="none" stroke="#0f3a7d" stroke-width="7" stroke-linecap="round"/>')
            out.append(f'<path d="{wpath}" fill="none" stroke="{BLUE}" stroke-width="5" stroke-linecap="round"/>')
    if hi:
        c, r = hi
        x = ox + c*sw; y = oy + r*sh
...
def dupstitch_panels(L):
    ...
    panels.append((3, L('Stikk nålen ned i samme hull som du kom opp. Ferdig! Den blå V-en ligger nå oppå den hvite masken.',
                        'Bring the needle down into the same hole you came up. Done! The blue V now lies on top of the white stitch.'), g3))
...
ph_no = make_page('LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;RO-BØTTEHATT BARN, BRODERT', 'LME STRIKK')
pages = []
...
<div class="coverbanner">
  ...
  <h1 class="covertitle">RO-BØTTEHATT<br>TIL BABY OG BARN, BRODERT</h1>
  <div class="cflag">''' + mini_flag(34) + '''</div>
</div>
...
''' + card('<p class="center">Samme RO-bøttehatt som oppskriften for voksne: hvit bomullshatt med det norske flagget og '
      '"RO" i blått foran, og to blå bølgeskvulp bak, med en solid blå brem som bølger i kanten. Her strikker du hele '
      'hoveddelen ensfarget hvit først, og legger til motivene til slutt med maskesting (duplikatsting), i stedet for å '
      'strikke dem inn underveis. Gradert helt fra bunnen av til tjueen babyer-, barne- og ungdomsstørrelser, 50 til 170. '
...
pages.append(ph_no(
    ...
    'først en blå brem som bølger nedover, deretter hele hoveddelen ensfarget hvit, og feller til slutt toppen ned til '
    'en liten rundet topp. Helt til slutt broderer du "RO" og flagget foran, og to bølger bak, med maskesting (kalles '
    'også duplikatsting), en søm som legger seg oppå strikken og ser strikket ut.</p>' +
    ...
    card(ul([
        ...
        'Å brodere to ulike motiver (RO+flagg foran, bølger bak) med maskesting etter rutediagram',
        'Å felle en rundet topp jevnt ned til få masker',
    ])) +
    pink('HVOR VANSKELIG ER DET?') +
    card('<p>Nybegynnervennlig. Du bør kunne legge opp, strikke glattstrikk rundt og bytte farge. Selve hoveddelen '
         'strikkes i kun hvitt, motivene kommer på etterpå med nål og tråd, og alt er forklart trinn for trinn.</p>') +
...
pages.append(ph_no(
    ...
    card('<p>Et glatt bomullsgarn (aran/tykkelse 4) som gir 17 masker x 22 omganger glattstrikk = 10 x 10 cm '
         ...
         f'<tr><td><span class="dot" style="background:{BLUE}"></span> Kongeblå</td><td>hele bremmen, broderi: RO, bølgene</td></tr>'
         f'<tr><td><span class="dot" style="background:{RED}"></span> Rød</td><td>broderi: flagget</td></tr>'
         f'<tr><td><span class="dot" style="background:{NAVY}"></span> Marineblå</td><td>broderi: flagget</td></tr></table>'
         '<p class="small">Ha rikelig av hvit hovedfarge (nesten hele hatten er strikket i hvitt) og litt '
         'kongeblått til bremmen, og litt kongeblått, rødt og marineblått til broderiet.</p>') +
...
pages.append(ph_no(
    ...
    steps([
        ...
        'den bølgete kanten.',
        'Bytt til hvit hovedfarge. Nå strikker du resten av hatten hvit og ensfarget, motivene kommer på '
        'etterpå med broderi.',
    ]) +
...
pages.append(ph_no(
    ...
    steps([
        'Etter sammenstrikkingsomgangen strikker du glattstrikk rundt i hvit hovedfarge, hele veien, uten noe '
        'mønster. Dette er nå hoveddelen av hatten, den delen som synes best.',
        'Strikk rett fram til hele hoveddelen måler målet i kolonnen "Til topp" i tabellen på neste side, '
        'målt fra sammenstrikkingsomgangen. Ikke tenk på motivene ennå, de broderer du på til slutt, se '
        'Del 3 på neste oppslag.',
    ], start=1) +
    tealp('TABELL: HOVEDDEL') +
    sizetable(['Str.', 'Masker (hoveddel)', 'Høyde til topp'], list(zip(SIZES, HOVEDDEL, TIL_TOPP))) +
    cream('<p class="creamtitle">Motivene broderes midt i hoveddelen i høyden, ikke helt nederst mot bremmen '
          'og ikke helt oppe ved toppen. Mer om det på neste side.</p>')
, 8))

pages.append(ph_no(
    banner('DEL 3: SLIK BRODERER DU MOTIVENE') +
    '<p>Hatten er nå ferdig strikket, helt hvit. Motivene legger du til med maskesting: en søm som legger '
...
pages.append(ph_no(
    banner('DIAGRAMMENE OG PLASSERINGEN') +
    tealp('SLIK PLASSERER DU MOTIVENE') +
    card('<p>Tell maskene rundt og finn midt foran (motsatt av maskemarkøren, som er midt bak). RO er 15 '
         'masker bredt, sentrer det rundt midt foran. Flagget broderes ca. 2 omganger over RO, midt på. '
         'Bølgene broderes på samme høyde som RO, én på hver side av midt bak, med litt hvit imellom.</p>') +
    ...
    '<p class="small">Les alle diagrammene nedenfra og opp, akkurat som maskene i strikketøyet. Hver rad '
    'broderes fra høyre mot venstre. Farget rute = brodér med den fargen. Stiplet hvit rute = hopp over, '
    'der får hovedfargen synes uten broderi.</p>'
, 10))
...
pages.append(ph_no(
    ...
    tealp('AVSLUTNING') +
    card('<p>Fest alle løse tråder godt på innsiden, både etter bremmen og etter broderiet. Kontroller at '
         'broderisømmene ligger løst og lat oppå maskene, ikke stramme.</p>') +
    tealp('STELL') +
    ...
    card(check([
        ...
        'RO og flagget er brodert midt foran, bølgene midt bak',
        'Alle broderisting ligger løst, ikke stramme',
        'Toppen er dratt sammen og godt festet',
    ])) +
    '<div class="congrats">Gratulerer, du har strikket og brodert din egen RO-bøttehatt!</div>' +
    byline('Renate Dahl') +
...
ph_en = make_page('LME KNITTING PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;RO BUCKET HAT KIDS, DUPLICATE STITCH', 'LME KNIT')
pages = []
...
<div class="coverbanner">
  <div class="cflag">''' + mini_flag(34) + '''</div>
  <h1 class="covertitle">RO BUCKET HAT<br>FOR BABY AND CHILD, DUPLICATE STITCH</h1>
  <div class="cflag">''' + mini_flag(34) + '''</div>
</div>
...
''' + card('<p class="center">The same RO bucket hat as the pattern for adults: a white cotton hat with the Norwegian '
      'flag and "RO" in blue at the front, and two blue waves at the back, with a solid blue brim that flares at the '
      'edge. Here you knit the whole main body in a single white colour first, then add the motifs at the end with '
      'duplicate stitch, instead of knitting them in as you go. Graded completely from scratch into twenty-one baby, '
      'child and teen sizes, 50 to 170. This pattern is complete on its own, you do not need any other LME pattern to '
      'knit it.</p>') + '''
...
pages.append(ph_en(
    ...
    'bottom up, in white main colour. You start with a blue brim that flares, then knit the whole main body '
    'in plain white, and finally decrease the crown down to a small rounded top. Right at the end you '
    'embroider "RO" and the flag at the front, and two waves at the back, with duplicate stitch (also called '
    'Swiss darning), a stitch that sits on top of the knitting and looks knitted in.</p>' +
    ...
    card(ul([
        ...
        'To embroider two different motifs (RO+flag at front, waves at back) with duplicate stitch, following charts',
    ...
    card('<p>Beginner friendly. You should be able to cast on, knit stockinette in the round and change '
         'colour. The main body is knitted in just white, the motifs come on afterwards with a needle and '
         'thread, and every step is spelled out in this pattern.</p>') +
...
pages.append(ph_en(
    ...
    card('<p>A smooth cotton yarn (aran weight) that gives 17 stitches x 22 rounds in stockinette = 10 x 10 '
         ...
         f'<tr><td><span class="dot" style="background:{BLUE}"></span> Royal blue</td><td>whole brim, embroidery: RO, the waves</td></tr>'
         f'<tr><td><span class="dot" style="background:{RED}"></span> Red</td><td>embroidery: the flag</td></tr>'
         f'<tr><td><span class="dot" style="background:{NAVY}"></span> Navy</td><td>embroidery: the flag</td></tr></table>'
         '<p class="small">Have plenty of white main colour (almost the whole hat is knitted in white) and a '
         'little royal blue for the brim, plus a little royal blue, red and navy for the embroidery.</p>') +
...
pages.append(ph_en(
    ...
    steps([
        ...
        'the flared edge.',
        'Switch to white main colour. From here you knit the rest of the hat plain white, the motifs come '
        'on afterwards with embroidery.',
    ]) +
...
pages.append(ph_en(
    ...
    steps([
        'After the decrease round, knit stockinette in the round in the white main colour, all the way, with '
        'no pattern. This is now the main body of the hat, the part that shows the most.',
        'Knit plain until the whole main body measures the "Height to top" value in the table on the next '
        'page, measured from the decrease round. Do not worry about the motifs yet, you embroider those on '
        'at the end, see Part 3 on the next spread.',
    ], start=1) +
    tealp('TABLE: MAIN BODY') +
    sizetable(['Size', 'Stitches (main body)', 'Height to top'], list(zip(SIZES, HOVEDDEL, TIL_TOPP))) +
    cream('<p class="creamtitle">The motifs are embroidered in the middle of the main body height, not right '
          'down against the brim and not right up at the top. More about that on the next page.</p>')
, 8))

pages.append(ph_en(
    banner('PART 3: HOW TO EMBROIDER THE MOTIFS') +
    '<p>The hat is now fully knitted, all white. You add the motifs with duplicate stitch: a stitch that '
...
pages.append(ph_en(
    banner('THE CHARTS AND PLACEMENT') +
    ...
    card('<p>Count the stitches around and find centre front (opposite the stitch marker, which sits at '
         'centre back). RO is 15 stitches wide, centre it around centre front. Embroider the flag about 2 '
         'rounds above RO, centred. Embroider the waves at the same height as RO, one on each side of '
         'centre back, with a little white in between.</p>') +
    ...
    f'<div class="chartrow">{chart_svg(WAVE, cell=17, numbers=True)}{chart_svg(WAVE_M, cell=17, numbers=True)}</div>' +
    '<p class="small">Read all charts from the bottom up, just like the stitches in the knitting. Work each '
    'row from right to left. Coloured square = embroider that colour. Dashed white square = skip it, the '
    'main colour shows through with no embroidery.</p>'
, 10))
...
pages.append(ph_en(
    ...
    card(check([
        ...
        'RO and the flag are embroidered at centre front, the waves at centre back',
        'All embroidered stitches lie loose, not tight',
        'The top is pulled tight and well fastened off',
    ])) +
    '<div class="congrats">Congratulations, you have knitted and embroidered your very own RO bucket hat!</div>' +
    byline('Renate Dahl') +
...
<title>RO-bøttehatt barn, brodert, LME strikkeoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages_no)}</body></html>'''
doc_en = f'''<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<title>RO bucket hat kids, duplicate stitch, LME knitting pattern</title>
<style>{css}</style></head>
<body>{''.join(pages_en)}</body></html>'''

(OUT_DIR / 'barn_strikk_brodert_ro_no.html').write_text(doc_no, encoding='utf-8')
(OUT_DIR / 'barn_strikk_brodert_ro_en.html').write_text(doc_en, encoding='utf-8')
