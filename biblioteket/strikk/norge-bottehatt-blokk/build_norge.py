...
PHOTO = pathlib.Path('/root/.claude/uploads/8a2defc1-69f7-5b9a-9a21-28d0d3730f69/709dfc1a-IMG_2685.png')
...
# ============ SIDE 1: FORSIDE ============
pages.append(page(f'''
<div class="coverimg"><img src="{photo_src}" alt="Rød NORGE-bøttehatt med flagg bak og bølget brem"></div>
...
{card('<p class="center">En sommerhatt i rød bomull med "NORGE" i store hvite blokkbokstaver rundt '
...
strikker du inn NORGE foran og flagget bak med flerfargestrikk, altså rett inn i hatten
...
{steps([
  ...
  'Nå kommer <b>mønsterbåndet</b>. Følg diagrammene: NORGE foran (side 9) og flagget bak (side 10). '
  'Foran og bak strikkes på de samme omgangene, med rødt imellom og på sidene.',
  'Både NORGE og flagget starter på <b>samme omgang</b>, den første i mønsterbåndet. NORGE er '
...
pages.append(page(f'''
{banner('DIAGRAM: NORGE')}
<p>En rute = en maske. Hvit rute = strikk med hvit. Rød rute = strikk med rød (bunnfargen).
NORGE går rundt forsiden i store blokkbokstaver. Les hver rad nedenfra og opp, og fra høyre
...
<div class="chartrow tight">
{chart_svg(LETTERS5['G'], cell=13)}{chart_svg(LETTERS5['E'], cell=13)}
</div>
{pink('HVOR PÅ HATTEN?')}
{card(f'<div class="stripwrap">{front_strip}</div>'
  + ul([
  '<b>Bortover:</b> NORGE sentreres midt foran (rett overfor maskemarkøren, som er midt bak). '
  'Tell 25 masker til venstre for midt foran; der starter N-en. La 1 maske rødt mellom bokstavene.',
  ...
  'Ordet er bredt og går litt rundt mot sidene. Det skal det, akkurat som på NORWAY-hatten!',
]))}
''', 9))
...
{card(ul([
  ...
  'NORGE foran, og fyller 10 omganger opp. Da står forsiden og baksiden like høyt.',
  'Mellom NORGE og flagget er det bare rødt på hver side. Der får bunnfargen hvile.',
]))}
''', 10))
...
{card(ul([
  ...
  'NORGE sitter foran og flagget bak',
...
<title>NORGE-bøttehatt (blokkbokstaver), LME strikkeoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages)}</body></html>'''

(BASE / 'hatt_norge.html').write_text(doc, encoding='utf-8')
