# -*- coding: utf-8 -*-
"""Genererer 3 separate LME-strikkeoppskrifter (NORGE / NORWAY / RO, hver for seg) for
bøttehatter barn, som erstatning for den gamle samle-PDF-en. Skriver HTML til hver
variants egen mappe (bottehatter-barn-strikk-<variant>/)."""
import base64, html, pathlib

BASE = pathlib.Path(__file__).parent
SRC = BASE / 'bottehatter-barn-strikk'
...
VARIANTS = [
    dict(
        slug='norge', is_striped=True,
        chart=NORGE_CHART, cmap=CMAP_LETTERS, cell=20, stitches=29,
        no_word='NORGE', no_title='NORGE-BØTTEHATT<br>TIL BABY OG BARN',
        no_desc='Ordet «NORGE» strikket i hvitt tvers over pannen, med en stripet brem i rødt/hvitt/marineblått.',
        en_word='NORGE', en_title='NORGE BUCKET HAT<br>FOR BABY AND CHILD',
        en_desc='The word &laquo;NORGE&raquo; knitted in white across the forehead, with a striped red/white/navy brim.',
    ),
    dict(
        slug='norway', is_striped=True,
        chart=NORWAY_CHART, cmap=CMAP_LETTERS, cell=17, stitches=35,
        no_word='NORWAY', no_title='NORWAY-BØTTEHATT<br>TIL BABY OG BARN',
        no_desc='Ordet «NORWAY» strikket i hvitt tvers over pannen, med en stripet brem i rødt/hvitt/marineblått.',
        en_word='NORWAY', en_title='NORWAY BUCKET HAT<br>FOR BABY AND CHILD',
        en_desc='The word &laquo;NORWAY&raquo; knitted in white across the forehead, with a striped red/white/navy brim.',
    ),
    dict(
        slug='ro', is_striped=False,
        chart=RO_FLAG_CHART, cmap=CMAP_ROFLAG, cell=20, stitches=25,
        no_word='RO', no_title='RO-BØTTEHATT<br>TIL BABY OG BARN',
        no_desc='Bokstavene «RO» pluss et lite norsk flagg strikket i hvitt/marineblått, med en ensfarget marineblå brem.',
        en_word='RO', en_title='RO BUCKET HAT<br>FOR BABY AND CHILD',
        en_desc='The letters &laquo;RO&raquo; plus a small Norwegian flag knitted in white/navy, with a solid navy brim.',
    ),
]
...
def build(v):
    slug = v['slug']
    out_dir = BASE / f'bottehatter-barn-strikk-{slug}'
    ph_no = make_page('LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;BØTTEHATT BARN', 'LME STRIKK')
    pages = []

    # ---- SIDE 1: FORSIDE ----
    pages.append(ph_no('''
<div class="coverimg"><img src="''' + photo_src + f'''" alt="{v['no_word']}-bøttehatt til baby og barn, strikket"></div>
...
''' + card(f'<p class="center">Samme bøttehatt som {v["no_word"]}-oppskriften for voksne, gradert helt fra bunnen av til '
      'tjueen babyer-, barne- og ungdomsstørrelser, 50 til 170. Egne, mindre bokstaver og et eget lite motiv er laget '
      'spesielt for de minste hodene. Denne oppskriften er komplett i seg selv, du trenger ikke eie noen '
      f'annen LME-oppskrift for å strikke den. {v["no_desc"]}</p>') + '''
...
''', 1))
    ...
    pages.append(ph_no(
        ...
        f'{"stripet" if v["is_striped"] else "ensfarget"} brem som bølger nedover, deretter hoveddelen rett opp med motivet i midten, og til slutt felles '
        'toppen ned til en liten rundet topp.</p>' +
    ...
    pages.append(ph_no(
        ...
        card('<p>Et glatt bomullsgarn (aran/tykkelse 4) som gir 17 masker x 22 omganger glattstrikk = 10 x 10 cm '
             ...
             f'<tr><td><span class="dot" style="background:{WHITE};border-color:#ccc;"></span> Hvit</td><td>bokstaver{", striper" if v["is_striped"] else ""}</td></tr>'
             f'<tr><td><span class="dot" style="background:{NAVY}"></span> Marineblå</td><td>{"striper, flagg" if v["is_striped"] else "hele bremmen, flagg"}</td></tr></table>'
    ...
    pages.append(ph_no(
        ...
        '<p>Motivet er strikket med hvitt eller marineblått på rød bunn, med teknikken flerfargestrikk (fair '
        ...
        '<p class="small">Les diagrammet nedenfra og opp. Fordi du strikker rundt, leses hver omgang fra '
    ...
    # ===================== ENGELSK =====================
    ph_en = make_page('LME KNITTING PATTERN&nbsp;&nbsp;|&nbsp;&nbsp;KIDS BUCKET HAT', 'LME KNIT')
    pages = []

    pages.append(ph_en('''
<div class="coverimg"><img src="''' + photo_src + f'''" alt="{v['en_word']} bucket hat for baby and child, knitted"></div>
...
''' + card(f'<p class="center">The same bucket hat as the {v["en_word"]} pattern for adults, graded completely '
      'from scratch into twenty-one baby, child and teen sizes, 50 to 170. Smaller letters and a small motif were '
      'designed just for the smallest heads. This pattern is complete on its own, you do not need any other '
      f'LME pattern to knit it. {v["en_desc"]}</p>') + '''
...
''', 1))
    ...
    pages.append(ph_en(
        ...
        f'bottom up. You start with a {"striped, flared" if v["is_striped"] else "solid-coloured"} brim, then the main body straight up with the motif in '
        'the middle, and finally decrease the crown down to a small rounded top.</p>' +
    ...
    pages.append(ph_en(
        ...
        card('<p>A smooth cotton yarn (aran weight) that gives 17 stitches x 22 rounds in stockinette = 10 x 10 '
             ...
             f'<tr><td><span class="dot" style="background:{WHITE};border-color:#ccc;"></span> White</td><td>letters{", stripes" if v["is_striped"] else ""}</td></tr>'
             f'<tr><td><span class="dot" style="background:{NAVY}"></span> Navy</td><td>{"stripes, flag" if v["is_striped"] else "whole brim, flag"}</td></tr></table>'
    ...
    pages.append(ph_en(
        ...
        '<p>The motif is knitted in white or navy on a red background, using stranded colourwork (fair isle): '
        ...
        f'<div class="chartrow">{chart_svg(v["chart"], v["cmap"], cell=v["cell"], numbers=True)}</div>' +
        '<p class="small">Read the chart from the bottom up. Because you are knitting in the round, each round '
...
<title>{v['no_word']}-bøttehatt barn, LME strikkeoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages_no)}</body></html>'''
    doc_en = f'''<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<title>{v['en_word']} bucket hat kids, LME knitting pattern</title>
<style>{css}</style></head>
<body>{''.join(pages_en)}</body></html>'''

    (out_dir / 'barn_strikk_no.html').write_text(doc_no, encoding='utf-8')
    (out_dir / 'barn_strikk_en.html').write_text(doc_en, encoding='utf-8')
