...
PHOTO = BASE / 'norway_strikk_ref.jpg'
...
def page(body, num, right_label='LME STRIKK'):
  ...
  <div class="phead">
    ...
    <div class="ph2">LME STRIKKEOPPSKRIFT&nbsp;&nbsp;|&nbsp;&nbsp;NORWAY-BØTTEHATT</div>
  </div>
...
# ============ SIDE 1: FORSIDE ============
pages.append(page(f'''
<div class="coverimg"><img src="{photo_src}" alt="Rød NORWAY-bøttehatt med flagg bak og bølget brem"></div>
...
<div class="subpill">STORE BLOKKBOKSTAVER, STRIKKET INN</div>
{card('<p class="center">En sommerhatt i rød bomull med "NORWAY" i store hvite blokkbokstaver rundt '
      'forsiden, og det norske flagget bak. Bokstavene og flagget er strikket rett inn i hatten med '
      'flerfargestrikk, ikke brodert på. Bremmen bølger som små skvulp i vannkanten. Passer barn, dame og herre. '
      'En fin heiahatt til fotball-VM!</p>')}
<div class="byline">
...
strikker du inn NORWAY foran og flagget bak med flerfargestrikk, altså rett inn i hatten
...
{card(steps([
  ...
  '<b>Hoveddelen med mønster:</b> du strikker rødt et lite stykke, og så strikker du inn NORWAY '
  'foran og flagget bak med to farger. Deretter rødt videre opp.',
...
<p>Flerfargestrikk betyr at du bruker to farger på samme omgang. Du strikker med rød der
diagrammet er rødt, og med hvit (eller blå) der diagrammet viser det. Fargen du ikke bruker,
...
{card(ul([
  ...
  'riktig vei.',
  'Ha rød på en finger og hvit på en annen, eller legg fra deg den ene og ta opp den andre. '
...
{steps([
  'Fortsett med rød og strikk rett rundt til hoveddelen måler <b>ca. 3 cm</b> fra bølgeomgangen. '
  'Da havner mønsteret pent i nedre del, som på bildet.',
  'Nå kommer <b>mønsterbåndet</b>. Følg diagrammene: NORWAY foran (side 9) og flagget bak (side 10). '
  'Foran og bak strikkes på de samme omgangene, med rødt imellom og på sidene.',
  'Både NORWAY og flagget starter på <b>samme omgang</b>, den første i mønsterbåndet. NORWAY er '
  'høyest med 11 omganger, flagget 10. Over flagget strikker du bare rødt den øverste omgangen.',
  'Når hele mønsterbåndet er strikket (11 omganger), klipp av hvit og blå og la ca. 5 cm henge '
  'på innsiden. Du fester endene til slutt.',
  'Fortsett med rød alene, rett rundt, til hoveddelen måler <b>9 (10) 11 cm</b> fra bølgeomgangen.',
])}
{cream('<p class="creamtitle">Tell alltid maskene før du starter mønsterbåndet. '
       'Riktig plassering er halve jobben!</p>')}
''', 8))

# ============ SIDE 9: DIAGRAM NORWAY ============
front_strip = strip_svg([
    ('N',9,'x'),('',1,'sp'),('O',9,'x'),('',1,'sp'),('R',9,'x'),('',1,'sp'),
    ('W',9,'x'),('',1,'sp'),('A',9,'x'),('',1,'sp'),('Y',9,'x'),
], 'Til sammen 59 masker bredt')
pages.append(page(f'''
{banner('DIAGRAM: NORWAY')}
<p>En rute = en maske. Hvit rute = strikk med hvit. Rød rute = strikk med rød (bunnfargen).
NORWAY går rundt forsiden i store blokkbokstaver. Les hver rad nedenfra og opp, og fra høyre
mot venstre. Hver bokstav er 9 ruter bred og 11 ruter høy.</p>
{tealp('BOKSTAVENE (9 RUTER BREDE, 11 RUTER HØYE)')}
<div class="chartrow tight">
{chart_svg(LETTERS5['N'], cell=13)}{chart_svg(LETTERS5['O'], cell=13)}{chart_svg(LETTERS5['R'], cell=13)}
</div>
<div class="chartrow tight">
{chart_svg(LETTERS5['W'], cell=13)}{chart_svg(LETTERS5['A'], cell=13)}{chart_svg(LETTERS5['Y'], cell=13)}
</div>
{pink('HVOR PÅ HATTEN?')}
{card(f'<div class="stripwrap">{front_strip}</div>'
  + ul([
  '<b>Bortover:</b> NORWAY sentreres midt foran (rett overfor maskemarkøren, som er midt bak). '
  'Tell 30 masker til venstre for midt foran; der starter N-en. La 1 maske rødt mellom bokstavene.',
  '<b>Oppover:</b> Alle bokstavene starter på den <b>første omgangen</b> i mønsterbåndet, og fyller '
  '11 omganger opp. Flagget bak starter på samme omgang.',
  'Ordet er bredt og går litt rundt mot sidene. Det skal det, akkurat som på den heklede utgaven!',
]))}
''', 9))

# ============ SIDE 10: DIAGRAM FLAGGET BAK ============
pages.append(page(f'''
{banner('DIAGRAM: FLAGGET BAK')}
<p>Midt bak på hatten strikker du inn det norske flagget. Den blå korsarmen skal gå ubrutt
gjennom, med hvit kant rundt. Les nedenfra og opp, fra høyre mot venstre.</p>
<div class="chartrow">
{chart_svg(FLAG, cell=26, numbers=True, title='FLAGGET (13 RUTER BREDT, 10 RUTER HØYT)')}
</div>
{pink('HVOR PÅ HATTEN?')}
{card(ul([
  '<b>Bortover:</b> Midt bak er ved maskemarkøren. Flagget er 13 masker bredt, så du teller 6 '
  'masker til venstre og 6 til høyre for markøren, og har markøren i den 7. masken (midt i flagget).',
  '<b>Oppover:</b> Flagget starter på den <b>første omgangen</b> i mønsterbåndet, samme omgang som '
  'NORWAY foran, og fyller 10 omganger opp. Da står forsiden og baksiden like høyt.',
  'Mellom NORWAY og flagget er det bare rødt på hver side. Der får bunnfargen hvile.',
]))}
''', 10))
...
pages.append(page(f'''
{banner('DEL 3: TOPPEN')}
<p>Mønsterbåndet er ferdig og du strikker med rød alene igjen. Nå skal hatten bli smalere og
...
{card(ul([
  ...
  'NORWAY sitter foran og flagget bak',
  'Bremmen bølger og toppen er rund',
  'Hatten er tørr og klar for sommer, sjø og softis',
]))}
{cream('<p class="creamtitle">Gratulerer, du har strikket din egen NORWAY-hatt!<br>'
...
<title>NORWAY-bøttehatt, LME strikkeoppskrift</title>
<style>{css}</style></head>
<body>{''.join(pages)}</body></html>'''

(BASE / 'hatt_norway.html').write_text(doc, encoding='utf-8')
