/**
 * Kurssider: /kurs/<adresse>
 *
 * Serverer alltid kursvisningen (kurs/kurs.html) for disse adressene,
 ...
 * (/kurs/kurs.html -> /kurs). Den maa foelges her paa serveren.
 * Sendes den til nettleseren, mister adressen kursnavnet, og man havner
 * paa kursoversikten i stedet for kurset.
 */
export async function onRequestGet(context) {
  let res = await context.env.ASSETS.fetch(new URL("/kurs/kurs.html", context.request.url));
  for (let hopp = 0; hopp < 3 && res.status >= 300 && res.status < 400; hopp++) {
    const til = res.headers.get("Location") || "/kurs";
