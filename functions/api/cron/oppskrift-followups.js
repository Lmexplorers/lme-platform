/**
 * Oppskrifter — sender oppfølgingsmail (dag 3 og uke 2) fra køen.
 *
 * Webhooken legger to oppfølgere i KV ved kjøp (opp_fu:<e-post>:d3 og :w2).
 * Dette endepunktet går gjennom køen og sender de som er modne
 * (sendAfter <= naa) via MailerSend, og fjerner dem etterpaa.
 *
 * Kalles daglig av GitHub Actions (.github/workflows/oppskrift-followups.yml).
 * Valgfri beskyttelse: sett env OPPSKRIFT_CRON_TOKEN, saa kreves ?token=... .
 *
 *   GET /api/cron/oppskrift-followups
 */
...
export async function onRequest(context) {
  ...
  let cursor;
  do {
    const list = await env.BUILDER_KV.list({ prefix: "opp_fu:", cursor: cursor });
    for (const k of list.keys) {
      ...
      const res = await sendOppskriftMail(env, {
        to: job.email, name: job.name, lang: job.lang, kind: job.kind, pid: job.pid,
      });
