/**
 * Delt tilgangssjekk for LME Content Studio-generering.
 *
...
export async function enforceGeneration(context, kind) {
  ...
  const user = await sessionUser(context);
  if (!user) {
    return { ok: false, status: 401, error: "Du må være logget inn for å bruke LME Content Studio." };
  }
  if (isOwner(user)) return { ok: true };
  const sub = await subscriptionFor(context, user);
  if (!sub || sub.status !== "active") {
    return { ok: false, status: 402, error: "Dette krever et aktivt LME Content Studio-abonnement. Se planene på /oppgrader." };
  }
