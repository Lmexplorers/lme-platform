...
/* Bygger nedlastings-HTML: én knapp for enkeltoppskrift, punktliste for pakke. */
function downloads(prod, lang) {
  ...
  const items = files.map((fl) =>
    '<li style="margin:6px 0;">' + link(fl.url, (lang === "en" ? fl.en : fl.no) || "PDF") + '</li>').join("");
