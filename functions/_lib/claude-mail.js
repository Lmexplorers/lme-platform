...
const VIDERE = SITE + "/academy/claude-videre";
...
/* Låste kurssider (js/course-gate.js) krever en personlig ?t=<token>-lenke,
   se grantCourseAccess i _lib/course-access.js. tokenUrl legger den på hvis
   den finnes (alltid tilfelle ved ekte kjøp; tom bare hvis noe feilet). */
  ...
  no: {
    takk: (name, token) => ({
      ...
      html: wrap(
        ...
        btn(tokenUrl(COURSE, token), "Åpne Claude-kurset") +
      ...
      text: "Hei " + (name || "") + ",\n\nTusen takk, og så gøy å ha deg med! Claude-kurset ditt er klart.\n\nÅpne kurset: " + tokenUrl(COURSE, token) + "\nOppskriftspakken (PDF): " + PDF + "\n\nVil du ta det videre? Videre med Claude: " + UPSELL_NO + "\n\nKlem fra Renate",
    }),
    "takk-videre": (name, token) => ({
      subject: "Takk for kjøpet, her er Videre med Claude 💛",
      html: wrap(
        '<p>Hei ' + esc(name) + ',</p>' +
        '<p>Tusen takk! Videre med Claude er klart, og du kan starte med en gang.</p>' +
        btn(tokenUrl(VIDERE, token), "Åpne Videre med Claude") +
        '<p>Her lærer du skills, koblinger og hvordan du kan bygge enkle sider selv, alt forklart i vanlig språk.</p>' +
        '<p>Har du spørsmål, svar på denne e-posten, så hjelper jeg deg.</p>' +
        '<p>Klem fra Renate</p>'
      ),
      text: "Hei " + (name || "") + ",\n\nTusen takk! Videre med Claude er klart.\n\nÅpne kurset: " + tokenUrl(VIDERE, token) + "\n\nKlem fra Renate",
    }),
    oppfolging: (name, token) => ({
  ...
  en: {
    takk: (name, token) => ({
      ...
      html: wrap(
        ...
        btn(tokenUrl(COURSE, token), "Open the Claude course") +
      ...
      text: "Hi " + (name || "") + ",\n\nThank you so much! Your Claude course is ready.\n\nOpen the course: " + tokenUrl(COURSE, token) + "\nRecipe pack (PDF): " + PDF + "\n\nNext Level with Claude: " + UPSELL_EN + "\n\nWarm wishes,\nRenate",
    }),
    "takk-videre": (name, token) => ({
      subject: "Thank you, here's Next Level with Claude 💛",
      html: wrap(
        '<p>Hi ' + esc(name) + ',</p>' +
        '<p>Thank you so much! Next Level with Claude is ready, and you can start right away.</p>' +
        btn(tokenUrl(VIDERE, token), "Open Next Level with Claude") +
        '<p>Here you\'ll learn skills, connections, and how to build simple pages yourself, all in plain language.</p>' +
        '<p>If you have any questions, just reply to this email and I\'ll help you.</p>' +
        '<p>Warm wishes,<br>Renate</p>'
      ),
      text: "Hi " + (name || "") + ",\n\nThank you so much! Next Level with Claude is ready.\n\nOpen the course: " + tokenUrl(VIDERE, token) + "\n\nWarm wishes,\nRenate",
    }),
    oppfolging: (name, token) => ({
...
export function claudeEmail(lang, kind, name, token) {
...
export async function sendClaudeMail(env, opts) {
  ...
  const msg = claudeEmail(opts.lang, opts.kind, opts.name, opts.token);
  const body = {
